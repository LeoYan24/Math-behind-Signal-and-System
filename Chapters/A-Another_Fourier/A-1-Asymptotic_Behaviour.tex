...
\begin{proof}
    ...
    & =\left(f(t)-\sum_{n=-N}^{N}c_n \mathrm{e}^{\mathrm{i}n\omega t}\right)\left(\overline{f(t)}-\sum_{n=-N}^{N}c_n \mathrm{e}^{-\mathrm{i}n\omega t}\right)\\
...
f的原函数F的傅里叶系数同理，并且只要\textbf{分段连续}（见\cref{def:piecewise_continuos}）即可保证f可积，但是我们必须保证F是周期函数，这要求f的直流分量为0：
