\section{施瓦兹函数类}\label{sec:Schwartz_Functions}

\subsection{施瓦兹函数在傅里叶变换下的封闭性}\label{sub:fourier_close}
在\refsub{schwartz}中，我们介绍了施瓦兹函数类及其对偶空间分布，并直接假定了施瓦兹函数类在傅里叶变换下的封闭性。这一小节将填补这个逻辑漏洞，并介绍施瓦兹函数类的一些其他好处。由于不涉及具体的计算，简洁起见，我们将在本节使用角频率$\omega$。
...
\begin{theorem}[施瓦兹函数类在傅里叶变换下的封闭性]
    \label{thm:schwartz_close}
    设$\varphi\in\mathcal{S}$，则$\mathcal{F} \varphi,\mathcal{F} ^{-1} \varphi\in\mathcal{S}$。
\end{theorem}
\begin{proof}
    只需证明$\lim_{|\omega|\to\infty}|\omega|^m \mathcal{F} \varphi^{(n)}(\omega)=0,\forall m,n\in\mathbb{N}$。由傅里叶变换的微分性质（\cref{thm:FT_derivative}），
    \begin{align*}
    ...
    \end{align*}
    根据黎曼-勒贝格引理（见\refsub{riemman_lebesgue_lemma}），\[\varphi^{(n)}\in L^1(\mathbb{R})\implies\lim_{|\omega|\to\infty}\mathcal{F}\left[ \varphi^{(n)}(\omega)\right]=0\]
...
我们来验证，紧支函数的傅里叶变换不再是紧支函数，从而一开始介绍的$\mathcal{C} $在傅里叶变换下不封闭。事实上，非零函数不可能既时限，又带限，即：
\begin{theorem}
    \label{thm:time_freq_limit}
    设$f\in L^1(\mathbb{R}),f\neq 0$，如果$f$在时域上紧支，则其傅里叶变换$\mathcal{F} f(\omega)$不可能在频域上紧支，反之亦然。
\end{theorem}

\begin{proof}
    我们假设$\supp(f)\in[-T/2,T/2],\supp(\mathcal{F}f)\in[-\Omega/2,\Omega/2]$，证明$f=0$。
    ...
    \end{align*}
    由于$\supp(f)\in[-T/2,T/2]$，所以$f(t)$在$|t|>T/2$时恒为0，故\[\int_{-\Omega/2}^{\Omega/2}\mathcal{F} f(\omega)\mathrm{e}^{\mathrm{i}\omega t}\,d\omega=0,\quad \forall |t|>T/2\]
    对以上恒等式两边同时求n阶导数，得到\[\int_{-\Omega/2}^{\Omega/2}(\mathrm{i}\omega)^n \mathcal{F} f(\omega)\mathrm{e}^{\mathrm{i}\omega t}\,d\omega=0,\quad \forall |t|>T/2\]
    ...
    \textbf{法二}：利用解析延拓的唯一性。设$\supp(f)\in[-T/2,T/2]$，则\begin{align*}
        \mathcal{F} f(\omega) & =\int_{-\infty}^{\infty}f(t)\mathrm{e}^{-\mathrm{i}\omega t}\,dt \\
                              & =\int_{-T/2}^{T/2}f(t)\mathrm{e}^{-\mathrm{i}\omega t}\,dt
    \end{align*}
    这是一个可导的实变函数，可以将其定义域向复平面$\mathbb{C}$做全纯延拓，这就是双边拉普拉斯变换（\cref{def:bi_laplace}）：
    \begin{align*}
        F(s)=\int_{-\infty}^{\infty}f(t)\mathrm{e}^{-st}\,dt=\int_{-T/2}^{T/2}f(t)\mathrm{e}^{-st}\,dt,\quad F(\mathrm{i}\omega)=\mathcal{F} f(\omega)
    \end{align*}
    它同样具备全纯性。由于积分区间有限，$f\in\mathcal{C} $光滑，以上积分总是存在的，并且可以对$s$求导，从而也是全纯的：\[F'(s)=-\int_{-T/2}^{T/2}tf(t)\mathrm{e}^{-st}\,dt\]
    即$F(s)$是整函数。但是，$F(s)=0,\forall s=\mathrm{i}\omega,|\omega|> \Omega/2$，由唯一延拓定理（\cref{cor:unique_continuation}），$F(s)=0,\forall s\in\mathbb{C}$，因此$f(t)=0$。
\end{proof}

%时域频域“带宽”积，海森堡不确定性原理
必须指出，实际应用中我们总是考虑时域、频域都近似紧支的函数，指出时限、带限不共存，只是理论需要。事实上，时域上的“宽度”与频域上的“宽度”之积存在一个下界，这一点从\cref{thm:FT_stretch}就能看出：
\lr{
...
\begin{proof}
    我们首先验证，$|f|^2$与$|F|^2$作为某个随机变量的概率密度函数是良定义的，从而其方差是良定义的。根据普朗歇尔定理（\cref{thm:plancherel}），\lr{
    ...
    我们假设$|f|^2$和$|F|^2$的数学期望为0，否则可以通过时移、频移定理将它调整为0，而不影响另一个域上信号的模值；又假设\[\int_{-\infty}^{\infty}t^2|f(t)|^2\,dt<\infty,\int_{-\infty}^{\infty}\xi^2|F(\xi)|^2\,d\xi<\infty\]
    即方差存在，则\begin{align*}
        4\pi^2 \sigma^2(f)\sigma^2(F) & =4\pi^2 \int_{-\infty}^{\infty}t^2|f(t)|^2\,dt\int_{-\infty}^{\infty}\xi^2|F(\xi)|^2\,d\xi                                        \\
                                      ...
                                      & =\frac{1}{4}\left(\evalat{t|f(t)|^2}{-\infty}{\infty}-\int_{-\infty}^{\infty}|f(t)|^2\,dt\right)^2           & \text{(分部积分)}
        %&=\frac{1}{4}&\text{(归一化)}
    \end{align*}
    上述过程中，我们通过普朗歇尔定理说明了$f'(t)\in L^2(\mathbb{R})$，因此$f(t)$几乎处处可导，$t|f(t)|^2$几乎处处连续，而期望$\int_{-\infty}^{\infty}t|f(t)|^2\,dt=0<\infty$，因此$\lim_{|t|\to\infty}t|f(t)|^2=0$；\\
    又已知$\int_{-\infty}^{\infty}|f(t)|^2\,dt=1$，因此，$\sigma(f)\sigma(F)\geq \dfrac{1}{4\pi}$，得证。
\end{proof}
...
\end{itemize}
我们已经证明过$\mathcal{F}$是线性的双射，下面来说明连续性。为此，我们需要给施瓦兹函数类赋予一个拓扑结构。尽管$\mathcal{S}\in L^p(\mathbb{R} )(1\leq p\leq\infty)$，但$L^p$空间的范数并不能很好地刻画施瓦兹函数类的性质，因此我们需要引入一族\textbf{半范数} (seminorm)：
...
\end{theorem}
\begin{proof}
    不妨设$\varphi=\mathcal{F} \varphi=0$。由傅里叶变换的微分性质（\cref{thm:FT_derivative}），
    \begin{align*}
        \mathcal{F}\left[\left((-\mathrm{i}t)^n\varphi_k\right)^{(m)}\right](\omega) & =(\mathrm{i}\omega)^m \mathcal{F}\left[(-\mathrm{i}t)^n \varphi_k\right](\omega)                            \\
        ...
        \|\mathcal{F} \varphi_k\|_{m,n}                                              & =\sup_{\omega\in\mathbb{R} }|\omega|^m\left|(\mathcal{F} \varphi_k)^{(n)}(\omega)\right|                    \\
                                                                                     & =\sup_{\omega\in\mathbb{R} }\left|\mathcal{F}\left[((-\mathrm{i}t)^n \varphi_k)^{(m)}\right](\omega)\right|
    \end{align*}
    令$\phi_k=\left((-\mathrm{i}t)^n \varphi_k\right)^{(m)}\in\mathcal{S} $，则
    \begin{align*}
    ...
    \begin{align*}
        |\phi_k(t)| & =\left|\left((-\mathrm{i}t)^n \varphi_k\right)^{(m)}(t)\right|                                     \\
    ...
    \begin{align*}
        |\phi_k(t)| & \leq \sum_{j=0}^{\min(m,n)}\binom{m}{j}\frac{n!}{(n-j)!}\frac{\|\varphi_k\|_{n-j+2,m-j}}{|t|^2} \\
                    & <\frac{C\epsilon}{|t|^2},\quad C=\sum_{j=0}^{\min(m,n)}\binom{m}{j}\frac{n!}{(n-j)!}
    \end{align*}
    因此有\begin{align*}
        \int_{|t|\geq 1}|\phi_k(t)|\,dt & \leq \int_{|t|\geq 1}\frac{C\epsilon}{|t|^2}\,dt \\
                                        & =2C\epsilon\int_{1}^{\infty}\frac{1}{t^2}\,dt    \\
                                        & =2C\epsilon
    \end{align*}
...
其实，如果我们考虑$L^1(\mathbb{R})\cap L^2(\mathbb{R})$中傅里叶变换的连续性，可以得到类似的结论，但证明要简单得多，因为$L^1(\mathbb{R})\cap L^2(\mathbb{R})$中有普朗歇尔定理（\cref{thm:plancherel}）。
\begin{theorem}[$L^1(\mathbb{R})\cap L^2(\mathbb{R})$上傅里叶变换的连续性]
    设
    $$\lim_{k\to\infty}\|f_k-f\|_2=0$$
    则
    $$\lim_{k\to\infty}\|\mathcal{F}f_k-\mathcal{F}f\|_2=0$$
\end{theorem}
\begin{remark}
    尽管$L^1(\mathbb{R})\cap L^2(\mathbb{R})$是比$\mathcal{S} $更大的空间，但这个定理有一定的局限性：其一，依$L^2$范数收敛不能保证依半范数收敛，即证得的收敛性会减弱；其二，$L^1(\mathbb{R})\cap L^2(\mathbb{R})$并不是傅里叶变换下的封闭空间，因此我们无法保证傅里叶逆变换的连续性。
\end{remark}
\begin{proof}
    由普朗歇尔定理，有
    \lr{
...
另外，在弱收敛的意义下，傅里叶变换同样是$\mathcal{T} =\mathcal{S} ' $上的线性拓扑自同构（\cref{thm:distribution_fourier_continuous}）。
