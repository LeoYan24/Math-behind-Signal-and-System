...
\end{theorem}
\begin{proof}
    只需证明$\lim_{|\omega|\to\infty}|\omega|^m \mathcal{F} \varphi^{(n)}(\omega)=0,\forall m,n\in\mathbb{N}$。由傅里叶变换的微分性质（见\refsub{general_distributions}），
    \begin{align*}
...
\begin{proof}
我们假设$\mathrm{supp}\ f\in[-T/2,T/2],\mathrm{supp}\ \mathcal{F} f\in[-\Omega/2,\Omega/2]$，证明$f=0$。
...
\end{align*}
由于$\mathrm{supp}\ f\in[-T/2,T/2]$，所以$f(t)$在$|t|>T/2$时恒为0，故\[\int_{-\Omega/2}^{\Omega/2}\mathcal{F} f(\omega)\mathrm{e}^{\mathrm{i}\omega t}\,d\omega=0,\forall |t|>T/2\]
...
\textbf{法二}：利用解析延拓的唯一性。设$\mathrm{supp}\ f\in[-T/2,T/2]$，则\begin{align*}
    \mathcal{F} f(\omega)&=\int_{-\infty}^{\infty}f(t)\mathrm{e}^{-\mathrm{i}\omega t}\,dt\\
    &=\int_{-T/2}^{T/2}f(t)\mathrm{e}^{-\mathrm{i}\omega t}\,dt
\end{align*}
这是一个可导的实变函数，可以将其做全纯延拓。我们在\refsub{introduction_of_laplace}中已经介绍过拉普拉斯变换，不过那时使用的是单边拉普拉斯变换，这里为了涵盖$\mathrm{supp}\ f\in[-T/2,T/2]$的情形，我们使用双边拉普拉斯变换：
\begin{align*}
...
%时域频域“带宽”积，海森堡不确定性原理
必须指出，实际应用中我们总是考虑时域、频域都近似紧支的函数，指出时限、带限不共存，只是理论需要。事实上，时域上的“宽度”与频域上的“宽度”之积存在一个下界，这一点从\refsub{fourier_operation_properties}中提出的伸缩定理就可以看出：
\lr{
...
\begin{proof}
    我们首先验证，$|f|^2$与$|F|^2$作为某个随机变量的概率密度函数是良定义的，从而其方差是良定义的。根据普朗歇尔定理（见\refsub{fourier_operation_properties}），\lr{
    ...
    %&=\frac{1}{4}&\text{(归一化)}
    \end{align*}
    上述过程中，我们通过普朗歇尔定理说明了$f'(t)\in L^2(\mathbb{R})$，因此$f(t)$几乎处处可导，$t|f(t)|^2$几乎处处连续，而期望$\int_{-\infty}^{\infty}t|f(t)|^2\,dt=0<\infty$，因此$\lim_{|t|\to\infty}t|f(t)|^2=0$；$\int_{-\infty}^{\infty}|f(t)|^2\,dt=1$，因此，$\sigma(f)\sigma(F)\geq \dfrac{1}{4\pi}$，得证。
\end{proof}
...
\end{theorem}
\begin{proof}
    不妨设$\varphi=\mathcal{F} \varphi=0$。由傅里叶变换的微分性质（见\refsub{general_distributions}），
    \begin{align*}
...
其实，如果我们考虑$L^1(\mathbb{R})\cap L^2(\mathbb{R})$中傅里叶变换的连续性，可以得到类似的结论，但证明要简单得多，因为$L^1(\mathbb{R})\cap L^2(\mathbb{R})$中有普朗歇尔定理（见\refsub{fourier_operation_properties}）。
...
另外，在弱收敛的意义下，傅里叶变换同样是$\mathcal{T} =\mathcal{S} ' $上的线性拓扑自同构。在\refsub{operation_of_distribution}中，我们已经证明过分布的傅里叶变换的连续性：\begin{theorem}[$\mathcal{T}$上傅里叶变换的连续性]
