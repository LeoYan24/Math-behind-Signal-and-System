\section{线性微分方程组与状态变量分析}\label{sec:Equation_System}
...
尽管还未引入这个定义，直观告诉我们，以上微分方程组是\textbf{常系数线性微分方程组}。通过适当的消元和代入，可以由\refeqn{3.3.1}得到二阶常系数线性微分方程\refeqn{3.3.2}；通过引入参量$u_C(t)$，又可以由\refeqn{3.3.2}得到微分方程组\refeqn{3.3.1}。我们将看到，\textbf{常系数线性的微分方程组与高阶微分方程是等价的}，它们的解法有诸多相通之处。

在上例中，$i_L(t),u_C(t)$是系统的\textbf{状态变量}，给出它们的表达式就完全描述了系统的状态，因此，单独研究这种微分方程组是很有意义的。我们将在接下来的三个小节中讨论微分方程组的解法并由此获得对高阶方程的进一步认识，再在\refsub{state_variable_analysis}中给出状态变量的严格定义，从而引出系统的可观性和可控性。

本节中，我们主要考虑实系数的微分方程，但容易推广到复系数的情况。
...
\begin{definition}[线性微分方程组]
    设实矩阵值函数$A(t)$和实向量值函数$\mathbf{f}(t)$在区间$(a,b)$上连续，则形如\begin{align*}
    \frac{d\mathbf{y}}{dt}=A(t)\mathbf{y}+\mathbf{f}(t)
    \end{align*}
    的微分方程组则称该微分方程组为\textbf{一阶线性微分方程组}，其中$\mathbf{f}(t)$为\textbf{非齐次项}。

    特别地，如果$\mathbf{f}(t)=\mathbf{0}$，则称之为\textbf{齐次线性微分方程组}；如果$A(t)$为常数矩阵，则称为\textbf{常系数线性微分方程组}。
...
称
$$\frac{d\mathbf{y}}{dt}=A(t)\mathbf{y}+\mathbf{f}(t)$$
是线性微分方程，是因为函数$A(t)\mathbf{y}$对$\mathbf{y}$的每个分量都是线性的。以$\mathbf{f}(t)$为系统的激励，以初始条件为零向量时的解作为这个系统的零状态响应，则容易看出系统是线性的，但由于系数矩阵$A(t)$随时间变化，系统不具备时不变性。
...
令$A=PJP^{-1}$，其中$J$为$A$的Jordan标准型，做换元$\mathbf{z}=P^{-1}\mathbf{y}$，则以上方程化为\begin{align*}
    \frac{d\mathbf{z}}{dt}=J\mathbf{z}+P^{-1}\mathbf{f}(t)
\end{align*}
$J$是块对角矩阵，每个块为一个Jordan块，可以通过代入消元的方式将上述方程组转化为高阶微分方程，每一次代入消元都将使方程的阶数加一（在下面的例子中将会看到这一点）。如果$J$仅由单个Jordan块组成，则这个代入消元的过程可以一直进行到最后，$n-1$次代入消元使得方程的阶数由1上升至$n$，因此\textbf{方程组的阶数的定义与它对应的高阶方程的阶数是相符的}，即能够相互转化的方程与方程组，阶数也相同。

在$J$中含有多个Jordan块的情况下，我们的确无法得到一个$n$阶的微分方程。这种情况表明原方程在适当的换元下可以拆解为几个相互独立的小方程组（即解耦合的子系统），每个小方程组对应一个Jordan块，可以转化为一个微分方程，其阶数与Jordan块的阶数相同，从而得到一系列阶数之和为$n$的微分方程。在这个意义下，方程组的阶数仍然与方程相同。特别地，$A$可对角化时方程组可以化为$n$个一阶方程。
...
\begin{definition}[柯西问题]
    带有初始条件$\mathbf{y}(t_0)=\mathbf{y}_0$的微分方程组$\frac{d\mathbf{y}}{dt}=A(t)\mathbf{y}+\mathbf{f}(t)$的求解问题，称为\textbf{柯西问题}(Cauchy problem)。
\end{definition}
...
\begin{claim}[Picard定理]
    ...
    （其中范数$\|\cdot\|$就是欧几里得空间$\mathbb{R}^d$中的向量长度），则柯西问题的解在$|t-t_0|\leq h$上\textbf{存在且唯一}，其中
...
Picard定理适用范围很广，但我们仅仅关注线性微分方程组，由于在其定义中要求了系数矩阵$A(t)$和非齐次项$\mathbf{f}(t)$关于$t$的连续性，这种方程在每个初值点$(t_0,\mathbf{y}_0)\in\mathbb{R} \times \mathbb{R}^n$处都满足Picard定理的条件，而初值点的小邻域内的唯一解，又提供了其他点处的额外的“初始条件”，事实上，我们总能将这个唯一的解延伸至整个时间轴上。

下面我们首先考虑齐次方程，再通过常数变易法来处理非齐次方程。

利用前一命题，可以进一步说明：
\begin{theorem}\label{thm:n_order_ODE}
...
这个同构映射具有优美的几何意义：如图\reffig{solution_space}所示，$\mathbb{R}\times\mathbb{R}^n$构成了一个$n+1$维的空间，取定超平面$t=t_0$，在这个超平面上每个点$\mathbf{y}_0$都对应着一个初值点$(t_0,\mathbf{y}_0)$，从而对应唯一的解，即图中的解曲线$\mathbf{y}=\mathbf{y}(t)$。显然，所有解曲线不相交，并且构成空间$\mathbb{R}^n\times\mathbb{R}$的一个划分。
...
\begin{definition}[向量值函数的线性相关性]
    称向量值函数$\mathbf{y}_1(t),\mathbf{y}_2(t),\cdots,\mathbf{y}_n(t)$在区间$(a,b)$上是\textbf{线性相关}的，如果存在不全为零的常数$c_1,c_2,\cdots,c_n$，使得
    \[c_1\mathbf{y}_1(t)+c_2\mathbf{y}_2(t)+\cdots+c_n\mathbf{y}_n(t)=0,\quad \forall t\in(a,b)\]
    否则称它们是\textbf{线性无关}的。
\end{definition}

从上一个定理可知，齐次线性方程组有$n$个线性无关的解，通解为\begin{align*}
    \mathbf{y}(t)=c_1\mathbf{y}_1(t)+c_2\mathbf{y}_2(t)+\cdots+c_n\mathbf{y}_n(t)
\end{align*}
将$\{\mathbf{y}_k(t)\}_{k=1}^n$称为齐次线性方程组的\textbf{基本解组}，它张成了$n$维的线性空间，因此只要给出一个基本解组，就相当于确定了齐次线性方程组的解空间。

由于同构映射保持向量的线性无关性，通过解空间与初值空间之间的同构映射$\mathbf{H}$，只要在$t=t_0$处这$n$个解的值$\mathbf{y}_1(t_0),\mathbf{y}_2(t_0),\cdots,\mathbf{y}_n(t_0)$线性无关，那么它们就应当对应着在同构意义下线性无关的解。事实上，通过Wronsky行列式可以证明，$\{\mathbf{y}_k(t)\}_{k=1}^n$线性无关的确就等价于在某一点$t_0\in(a,b)$处，$\mathbf{y}_1(t_0),\mathbf{y}_2(t_0),\cdots,\mathbf{y}_n(t_0)$线性无关，这解释了以上定义的合理性。
...
\begin{definition}[Wronsky行列式]
    ...
    \vdots\\
    y_{1n}(t)
    \end{bmatrix},\mathbf{y}_2(t)=\begin{bmatrix}
    y_{21}(t)\\
    y_{22}(t)\\
    \vdots\\
    y_{2n}(t)
    \end{bmatrix},\cdots,\mathbf{y}_n(t)=\begin{bmatrix}
    y_{n1}(t)\\
    y_{n2}(t)\\
    \vdots\\
    y_{nn}(t)
    ...
    y_{11}(t) & y_{21}(t) & \cdots & y_{n1}(t)\\
    y_{12}(t) & y_{22}(t) & \cdots & y_{n2}(t)\\
    \vdots & \vdots & \ddots & \vdots\\
    y_{1n}(t) & y_{2n}(t) & \cdots & y_{nn}(t)
    \end{bmatrix}\]
\end{definition}

\begin{theorem}[Liouville公式]
    设$\mathbf{y}_1(t),\mathbf{y}_2(t),\cdots,\mathbf{y}_n(t)$为方程$\dfrac{d\mathbf{y}}{dt}=A(t)\mathbf{y}$的解，$W(t)$为它们的Wronsky行列式，则$\forall t_0\in (a,b)$，有
    \begin{align*}
        W(t)=W(t_0)\mathrm{e}^{-\int_{t_0}^t \mathrm{tr}A(s)\,ds},\quad \forall t\in(a,b)
    \end{align*}
\end{theorem}

这个定理的证明比较繁琐，可以在\cite{ode}等常微分方程的教材中找到，此处略去证明。它表明，$W(t)$要么在整个区间$(a,b)$上恒为零，要么在整个区间$(a,b)$上恒不为零。因此，\begin{align*}
    &\{\mathbf{y}_k(t)\}_{k=1}^n\text{线性无关}\\
    \iff& \forall t\in(a,b),W(t)\neq 0\\
    \iff& \exists t_0\in(a,b),W(t_0)\neq 0\\
    \iff& \exists t_0\in(a,b),\{\mathbf{y}_k(t_0)\}_{k=1}^n\text{线性无关}
\end{align*}
...
考虑由齐次线性微分方程转化而来的齐次线性微分方程组，则我们实际上只需要向量值函数$\mathbf{y}_1(t),\mathbf{y}_2(t),\cdots,\mathbf{y}_n(t)$的第一个分量，而后续的分量是前一分量的导数，第一个分量线性相关，则后续分量线性相关。这启发我们对数量值函数定义：
\begin{definition}[数量值函数的线性相关性]
    函数$y_1(t),y_2(t),\cdots,y_n(t)$在区间$(a,b)$上是\textbf{线性相关}的，如果存在不全为零的常数$c_1,c_2,\cdots,c_n$，使得
    \[c_1y_1(t)+c_2y_2(t)+\cdots+c_ny_n(t)=0,\quad \forall t\in(a,b)\]
    否则称它们是\textbf{线性无关}的。
\end{definition}

齐次线性微分方程的解空间是$n$维的，因此只要给出$n$个线性无关的解$y_1(t),y_2(t),\cdots,y_n(t)$，就能得到通解：\begin{align*}
    y(t)=c_1y_1(t)+c_2y_2(t)+\cdots+c_ny_n(t),\forall \mathbf{c}\in\mathbb{R}^n
\end{align*}

我们同样将$n$个线性无关的解$\{y_k(t)\}_{k=1}^n$称为齐次线性微分方程的\textbf{基本解组}。

\begin{definition}[Wronsky行列式]
    函数$y_1(t),y_2(t),\cdots,y_n(t)$的Wronsky行列式定义为
    \[W(t)=\det\begin{bmatrix}
...
我们知道，这种方程组的矩阵具有特殊的形式，它的迹恰为原方程的$-a_1(t)$，代入Liouville公式得到\begin{align*}
    W(t)=W(t_0)\mathrm{e}^{-\int_{t_0}^t a_1(s)\,ds},\quad\forall t\in(a,b)
\end{align*}

可见，对于齐次线性微分方程，与方程组类似，\begin{align*}
    &\{y_k(t)\}_{k=1}^n\text{线性无关}\\
    \iff& \forall t\in(a,b),W(t)\neq 0\\
    \iff& \exists t_0\in(a,b),W(t_0)\neq 0\\
    \iff& \exists t_0\in(a,b),\{y_k(t_0)\}_{k=1}^n\text{线性无关}
\end{align*}
...
\begin{definition}[解矩阵]
    ...
    \end{align*}
    的解组$\mathbf{y}_1(t),\mathbf{y}_2(t),\cdots,\mathbf{y}_n(t)$为列向量的矩阵称为\textbf{解矩阵}(solution matrix)：
    \[\phi(t)=[\mathbf{y}_1(t),\mathbf{y}_2(t),\cdots,\mathbf{y}_n(t)]\]
    解矩阵满足\begin{align*}
        \frac{d\phi(t)}{dt}=A(t)\phi(t)
    \end{align*}
    如果$\{\mathbf{y}_k(t)\}_{k=1}^n$是基本解组，则解矩阵$\phi(t)$是可逆的，称之为\textbf{基本解矩阵}(fundamental solution matrix)。
\end{definition}
\begin{proposition}
...
得到
\begin{align*}
    \phi(t)\frac{d\mathbf{c}(t)}{dt}=\mathbf{f}(t)
\end{align*}
由于基本解矩阵可逆，有
\begin{align*}
    \frac{d\mathbf{c}(t)}{dt}=\phi^{-1}(t)\mathbf{f}(t)
\end{align*}
积分得到
\begin{align*}
    \mathbf{c}(t)=\int_{t_0}^t \phi^{-1}(s)\mathbf{f}(s)\,ds
\end{align*}
因此，非齐次方程组有特解
\begin{align*}
    \mathbf{y}(t)=\phi(t)\int_{t_0}^t \phi^{-1}(s)\mathbf{f}(s)\,ds
\end{align*}
\begin{theorem}
    设$\phi(t)$是齐次线性微分方程组
    $$\frac{d\mathbf{y}}{dt}=A(t)\mathbf{y}$$
    ...
    $$\frac{d\mathbf{y}}{dt}=A(t)\mathbf{y}+\mathbf{f}(t)$$
    的通解为\begin{align*}
    \mathbf{y}(t)=\phi(t)\left(\mathbf{c}+\int_{t_0}^t \phi^{-1}(s)\mathbf{f}(s)\,ds\right)
    \end{align*}
\end{theorem}

\begin{corollary}
    非齐次方程组满足初始条件$\mathbf{y}(t_0)=\mathbf{y}_0$的解为\begin{align*}
    \mathbf{y}(t)=\phi(t)\left(\phi^{-1}(t_0)\mathbf{y}_0+\int_{t_0}^t \phi^{-1}(s)\mathbf{f}(s)\,ds\right)
    \end{align*}
\end{corollary}
...
本小节的最后，我们来看以上理论对于由方程转化而来的方程组的应用。我们已经知道，$n$阶方程的解空间是$n$维的，因此只要给出$n$个线性无关的解$y_1(t),y_2(t),\cdots,y_n(t)$，就能得到通解；线性无关性可以通过Wronsky行列式来判断；给出$n$个无关的条件，就能唯一确定一个解，特别地，这个条件经常为初始条件，它包含函数及其前$n-1$阶导数在某一点处的值。

现在，我们只关心$\mathbf{y}(t)$中第一个分量的值。根据前一定理可得：
\begin{theorem}
    设齐次方程的基本解组为$\{y_k(t)\}_{k=1}^n$，则非齐次方程\begin{align*}
    y^{(n)}+a_{n-1}(x)y^{(n-1)}+\cdots+a_1(x)y'+a_0(x)y=f(t)
    \end{align*}
    的通解为\begin{align*}
    y(t)=\sum_{k=1}^n c_k y_k(t)+\sum_{k=1}^n y_k(t)\int_{t_0}^t \frac{W_k(s)}{W(s)}f(s)\,ds
    \end{align*}
    其中$W(t)$是$\{y_k(t)\}_{k=1}^n$的Wronsky行列式，$W_k(t)$是$W(t)$的第$n$行第$k$列的元素的代数余子式。
\end{theorem}
\begin{proof}
    已知\begin{align*}
        \phi(t)=\begin{bmatrix}
        ...
        \vdots & \vdots & \ddots & \vdots\\
        y_1^{(n-1)}(t) & y_2^{(n-1)}(t) & \cdots & y_n^{(n-1)}(t)
        \end{bmatrix},\quad\mathbf{f}(t)=\begin{bmatrix}
        0\\
        \vdots\\
        0\\
        f(t)
    ...
    其中“*”表示我们不关注的元素，出现这种元素是因为只需要计算$\mathbf{y}$的第一个分量。代入前一定理进行计算（提示：后两个等号都在对明确列出所有元素的矩阵与列向量之积进行运算）：\begin{align*}
        &\phi(t)\int_{t_0}^t \phi^{-1}(s)\mathbf{f}(s)\,ds\\
        ...
        * & * & \cdots & *
        ...
        W_n(s)
        \end{bmatrix}\,ds\\
        =&\sum_{k=1}^n y_k(t)\int_{t_0}^t \frac{W_k(s)}{W(s)}f(s)\,ds
    \end{align*}
\end{proof}

然而，寻找基本解组是困难的，这一结果更多用于理论推导。本书不单独讨论高阶线性方程的求解技术，对于更特殊的常系数线性方程，上一节中展示的方程求解办法就已经较为完善了，而进一步的求解方法需要用到拉普拉斯变换，见\refsec{Laplace_ODE}。%将方程对应到方程组的意义，从数学上讲，是为了得到对解空间的理解和好用的Wronsky行列式；从信号与系统的角度来讲，则是为了导出系统的状态变量分析这一主题。
...
\end{align*}
进行计算。不过，本书不打算详细讲解这种计算方法，因为在\refsec{Laplace_ODE}中我们将使用拉普拉斯变换给出更为简洁有效的计算方式。对此感兴趣的读者可以参考常微分方程\cite{ode}或矩阵论的教材。
...
\begin{theorem}
    ...
    的通解为\begin{align*}
    \mathbf{y}(t)=\mathrm{e}^{At}\left(\mathbf{c}+\int_{t_0}^t \mathrm{e}^{-As}\mathbf{f}(s)\,ds\right)=\mathrm{e}^{At}\mathbf{c}+\int_{t_0}^t \mathrm{e}^{A(t-s)}\mathbf{f}(s)\,ds
    \end{align*}
\end{theorem}

\begin{corollary}\label{cor:differential_equation_system}
    非齐次方程组满足初始条件$\mathbf{y}(t_0)=\mathbf{y}_0$的解为\begin{align*}
    \mathbf{y}(t)=\mathrm{e}^{A(t-t_0)}\mathbf{y}_0+\int_{t_0}^t \mathrm{e}^{A(t-s)}\mathbf{f}(s)\,ds
    \end{align*}
\end{corollary}
...
以上公式同样可以用线性时不变系统的视角来解释：如果以$\mathbf{f}(t)$作为系统的激励，$\mathbf{y}(t)$作为系统的响应，则以上公式中前半部分为零输入响应，\textbf{后半部分为零状态响应，它是基本解矩阵与激励的“卷积”}。

类似于\refsub{fundamental_solution}中用基本解得到的方程的解的公式，\begin{align*}
    \mathbf{y}(t)=\mathrm{e}^{A(t-t_0)}\mathbf{y}_0+\int_{t_0}^t \mathrm{e}^{A(t-s)}\mathbf{f}(s)\,ds
\end{align*}
中给出的是基本解矩阵$\mathrm{e}^{At}$与非齐次项$\mathbf{f}(t)$在有限区间上的“卷积积分”。称之为卷积，一方面是因为激励是在$t_0$时刻才开始施加于系统的，应当认为$\mathbf{f}(t)$在$t<t_0$时为零；另一方面，充当“基本解”这个角色的实际上是$\phi(t)u(t)$，这样，积分限就可以由$(-\infty,\infty)$改为$(t_0,t)$。

\subsection{*连续系统的状态变量分析}\label{sub:state_variable_analysis}

\begin{definition}[状态变量]
    \textbf{状态变量}(state variable)是指能够唯一确定系统状态的最小的一组变量。只要知道$t=t_0$时刻的状态变量值及$t\geq t_0$时的激励，就能完全确定系统在$t\geq t_0$时的行为，包括状态$\mathbf{q}(t)$、状态变化率$\dfrac{d\mathbf{q}}{dt}$和响应$\mathbf{y}(t)$。
...
\end{align*}
因此又将方程的基本解矩阵$\mathrm{e}^{At}$称为\textbf{状态转移矩阵}(state transition matrix)，它与激励共同决定了状态矢量在状态空间中的移动情况。

考虑将系统矩阵做Jordan分解：$A=PJP^{-1}$，则状态矢量应该变为$\mathbf{z}(t)=P^{-1}\mathbf{q}(t)$，描述系统的方程变为：\begin{align*}
...
特别地，如果$A$是可对角化的，则每个子系统都是一阶的，状态变量的方程为\begin{align*}
    \frac{dz_k}{dt}=\lambda_k z_k(t)+\mathbf{p}_k^T B\mathbf{e}(t),\quad k=1,2,\cdots,n
\end{align*}
...
\begin{example}
    ...
    z_2(0)=0,z_2(T)=z_{2f},\quad \int_{0}^T \mathrm{e}^{-\lambda s}z_2(s)\,ds=z_{1f}\mathrm{e}^{-\lambda T}
    \end{align*}
...
可见，\textbf{每个模态的最后一个状态变量（链尾）与输入耦合，就等价于该模态是可控的}。
...
可见，\textbf{只要模态内的第一个状态变量（链首）与输出耦合，就等价于该模态是可观的}。
...
\end{theorem}
\begin{proof}
    不妨设起始状态为$\mathbf{q}(0-)=\mathbf{0}$。根据状态变量的解的公式，\begin{align*}
