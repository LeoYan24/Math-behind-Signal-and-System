...
然而，寻找基本解组是困难的，这一结果更多用于理论推导。本书不单独讨论高阶线性方程的求解技术，对于更特殊的常系数线性方程，上一节中展示的方程求解办法就已经较为完善了，而进一步的求解方法需要用到拉普拉斯变换，见\ref{sec:Laplace_ODE}。%将方程对应到方程组的意义，从数学上讲，是为了得到对解空间的理解和好用的Wronsky行列式；从信号与系统的角度来讲，则是为了导出系统的状态变量分析这一主题。
...
类似于\ref{sub:fundamental_solution}中用基本解得到的方程的解的公式，\begin{align*}
    \mathbf{y}(t)=\mathrm{e}^{A(t-t_0)}\mathbf{y}_0+\int_{t_0}^t \mathrm{e}^{A(t-s)}\mathbf{f}(s)\,ds
\end{align*}
