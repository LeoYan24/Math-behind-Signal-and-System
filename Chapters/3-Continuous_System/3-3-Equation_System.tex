...
在$J$中含有多个Jordan块的情况下，我们的确无法得到一个$n$阶的微分方程。这种情况表明原方程在适当的换元下可以拆解为几个相互独立的小方程组（即解耦合的子系统），每个小方程组对应一个Jordan块，从而可以转化为一个高阶微分方程，从而得到一系列阶数之和为$n$的微分方程。在这个意义下，方程组的阶数仍然与方程相同。特别地，$A$可对角化时方程组可以化为$n$个一阶方程。
...
\begin{claim}[Picard定理]
    ...
    （其中范数就是高维欧几里得空间$\mathbb{R}^d$中的向量长度），则柯西问题的解在$|t-t_0|\leq h$上\textbf{存在且唯一}，其中
