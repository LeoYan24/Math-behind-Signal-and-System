...
\begin{theorem}[Liouville公式]
    设$\mathbf{y}_1(t),\mathbf{y}_2(t),\cdots,\mathbf{y}_n(t)$为方程$\frac{d\mathbf{y}}{dt}=A(t)\mathbf{y}$的解，$W(t)$为它们的Wronsky行列式，则$\forall t_0\in (a,b)$，有
    \begin{align*}
...
\begin{proof}
    已知\begin{align*}
        ...
        \vdots & \vdots & \ddots & \vdots\\
        y_1^{(n-1)}(t) & y_2^{(n-1)}(t) & \cdots & y_n^{(n-1)}(t)
        \end{bmatrix},\mathbf{f}(t)=\begin{bmatrix}
        0\\
        \vdots\\
        0\\
        f(t)
