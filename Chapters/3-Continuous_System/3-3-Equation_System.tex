...
\begin{definition}[线性微分方程组]
    设实矩阵函数$A(t)$和实向量值函数$\mathbf{f}(t)$在区间$(a,b)$上连续，则形如\begin{align*}
...
以上公式同样可以用线性时不变系统的视角来解释：如果以$\mathbf{f}(t)$作为系统的激励，$\mathbf{y}(t)$作为系统的响应，则以上公式中前半部分为零输入响应，\textbf{后半部分为零状态响应，它是单位脉冲响应与激励的卷积}。为了说明这一点，我们首先定义矩阵值函数的卷积：
...
类似于用基本解得到的方程的解的公式，公式中给出的是基本解矩阵$\mathrm{e}^{At}$与非齐次项$\mathbf{f}(t)$在有限区间上的“卷积积分”。称之为卷积，一方面是因为，施加一个向量形式的冲击项作为激励（非齐次项），$t\geq 0$时才会有响应，即单位脉冲响应为$\mathrm{e}^{At}u(t)$（这正是基本解矩阵这样命名的原因之一，代入方程容易验证它是单位脉冲响应）；另一方面是因为激励是在$t_0$时刻才开始施加于系统的，这样，积分限就可以由$(-\infty,\infty)$改为$(t_0,t)$。
