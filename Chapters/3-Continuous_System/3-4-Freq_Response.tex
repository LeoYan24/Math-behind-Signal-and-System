...
在上一节中我们得到了$r(t)=(h*e)(t)$。在$r(t),e(t),h(t)\in L^1(\mathbb{R})$的条件下，利用傅里叶变换的卷积定理，有：
\[R(\omega)=H(\omega)E(\omega)\]
\begin{definition}[频率响应]
    将$H(\omega)$称为$r(t)$的\textbf{频率响应}，简称\textbf{频响}。
\end{definition}

将频率响应$H(\omega)$看作一个复数，即$H(\omega)=\left|H(\omega)\right|\mathrm{e}^{\mathrm{i}\varphi(\omega)}$，则$\left|H(\omega)\right|$表示系统对频率为$\omega$的信号成分的放大倍数，$\varphi(\omega)$表示该频率成分经过系统后相位的变化。我们定义:
\begin{definition}[幅频特性和相频特性]
    $\left|H(\omega)\right|-\omega$称为系统的\textbf{幅频特性}，$\varphi(\omega)-\omega$称为系统的\textbf{相频特性}，这里$\varphi(\omega)$通常取主值，即$(-\pi,\pi]$内的值。
\end{definition}

以下命题得自傅里叶变换的对偶性：
\begin{proposition}
    单位脉冲响应为实信号的线性时不变系统的频率响应满足$H(-\omega)=\overline{H(\omega)}$，这表明幅频特性$\left|H(\omega)\right|$是偶函数，相频特性$\varphi(\omega)$是奇函数。
\end{proposition}

一个物理可实现的线性时不变系统必须是因果的，即$h(t)=0,t<0$。此外，系统的频率响应$H(\omega)$还必须满足\textbf{佩利-维纳准则}(Paley-Wiener criterion)\cite{signal_systems}：
\begin{definition}[佩利-维纳准则]\label{def:paley_winner}
    可实现的因果系统，其频率响应$H(\omega)$必然满足：
    \[\int_{-\infty}^{\infty}\frac{\left|\ln\left|H(\omega)\right|\right|}{1+\omega^2}\,d\omega<\infty\]
\end{definition}
\begin{definition}[能量有限系统]
    如果系统单位脉冲响应$h(t)\in L^1(\mathbb{R})$，频率响应$H(\omega)$满足：
    $$\int_{-\infty}^{\infty}\left|H(\omega)\right|^2\,d\omega<\infty$$
    即$H(\omega)\in L^2(\mathbb{R})$，就称该系统是\textbf{能量有限}的。
\end{definition}
通常来说，我们希望系统是能量有限的，因为在输入冲击$\delta$时，输出为$h(t)$，根据帕塞瓦尔定理（\cref{thm:plancherel}），$h(t)$与$H(\omega)$的能量仅相差一个常系数$2\pi$，所以上述条件实际上是要求单位脉冲响应$h(t)$能量有限。
\begin{claim}
    对于因果系统，如果$h(t)\in L^1(\mathbb{R})$，则能量有限是满足佩利-维纳准则的充分不必要条件：
    \[\int_{-\infty}^{\infty}\left|H(\omega)\right|^2\,d\omega<\infty\implies\int_{-\infty}^{\infty}\frac{\left|\ln\left|H(\omega)\right|\right|}{1+\omega^2}\,d\omega<\infty\]
\end{claim}

佩利-维纳准则表明系统的对数幅度响应$\left|\ln\left|H(\omega)\right|\right|$不能“陡峭截止”：
\begin{example}
    高斯滤波器的单位冲激响应和频率响应特性分别为
    $$h(t)=\mathrm{e}^{-t^2},\quad H(\omega)=\sqrt{\pi}\mathrm{e}^{-\omega^2/4}$$
    利用反常积分的比较判别法的极限形式，
    \[\frac{|\ln\left|H(\omega)\right||}{1+\omega^2}=\frac{\left|\ln \sqrt{\pi}-\frac{\omega^2}{4}\right|}{1+\omega^2}\sim \frac{\omega^2/4}{\omega^2}=\frac{1}{4},\quad |\omega|\to\infty\]
    于是
    \[\int_{-\infty}^{\infty}\frac{\left|\ln\left|H(\omega)\right|\right|}{1+\omega^2}\,d\omega=\infty\]
    高斯滤波器不满足佩利-维纳准则，且不是因果系统，因此不是可实现的。
\end{example}

佩利-维纳准则表明$\left|H(\omega)\right|$是能量有限的，并且衰减速度不能过快，特别地，$\left|H(\omega)\right|$不能紧支，于是下一小节中提到的理想滤波器（\cref{def:ideal_filter}）都是不可能实现的。要求能量有限，是因为有限能量输入对应有限能量输出。激励$e(t)=\delta(t)$时，输入能量是有限的，$h(t)$的能量也是有限的，根据帕塞瓦尔定理，$H(\omega)$的能量也是有限的；另一方面，$H\in L^2(\mathbb{R})$是得到佩里维纳准则的第二条所必需的。
...
值得一提的是，限制$h(t)\in L^1(\mathbb{R})$有时是不必要的。根据分布与函数的卷积定理（\cref{thm:distribution_func_conv}），在
$$e(t)\in\mathcal{S},h(t)\in\mathcal{T},\text{或}e(t)\in\mathcal{T},h(t)\in\mathcal{S}$$
的情况下，同样有$Y(\omega)=H(\omega)X(\omega)$，例如，$h(t)=\delta+\delta_{1}\in\mathcal{T}$就是一个将信号与它时延$1s$所得信号相加的系统。

\subsection{滤波器}\label{sub:filter}
我们曾在\refsub{convolution}中提到，滤波就是把信号中的某些成分剔除或大幅减少，相当于在频域乘以一个函数，在时域上的表现则是与这个函数的傅里叶逆变换进行卷积。从频率响应的视角来看，滤波器就是一个系统，理想滤波器的幅频特性在某些频率处接近于0，而在另一些频率处接近于1。

\begin{definition}[滤波器的分类]
    \label{def:ideal_filter}
    \textbf{低通滤波器}是指幅频特性$\left|H(\omega)\right|$近似等于$$\chi_{[-\omega_c,\omega_c]}$$的滤波器，$\omega_c$为截止频率；\textbf{高通滤波器}则相反，幅频特性近似$$1-\chi_{[-\omega_c,\omega_c]}$$
...
\begin{definition}[特征函数]\label{def:eigenfunction}
    给定$\omega_0$，将函数$\mathrm{e}^{\mathrm{i}\omega_0 t}$称为系统H的\textbf{特征函数}(eigenfunction)，\textbf{特征值}(eigenvalue)为$H(\omega_0)$。
\end{definition}
...
可以看到，$\left|H(\omega)\right|=K,t_0=-\varphi(\omega)/\omega$，其幅频特性是常数，图像为一水平线；相频特性是线性函数，图像为过原点的直线，斜率$-t_0$对应系统造成的延时。

由于物理限制，我们不可能实现一个无失真传输系统，但只要所讨论的信号的频谱基本都集中在特定的频段当中，就可以近似认为在这个频段上满足$\left|H(\omega)\right|=K,\varphi(\omega)=-\omega t_0$的系统是无失真传输系统。
...
\begin{example}[相时延]\label{exmp:phase_delay}
设输入信号为单频复指数信号$\mathrm{e}^{\mathrm{i}\omega_0 t}$，求频率响应为$H(\omega)$
的线性时不变系统的输出信号$r(t)$。\\
解：我们已经在前面得到：\[r(t)=H\left[\mathrm{e}^{\mathrm{i}\omega_0 t}\right]=H(\omega_0)\mathrm{e}^{\mathrm{i}\omega_0 t}=\left|H(\omega_0)\right|\mathrm{e}^{\mathrm{i}(\omega_0 t+\varphi(\omega_0))}\]
可以看到，输出信号仍然是单频复指数信号，其幅值被放大了$|H(\omega_0)|$倍，相位
延迟了$\varphi(\omega_0)$，折合到时间上，就得到相时延：
$$r(t)=\left|H(\omega_0)\right|\mathrm{e}^{\mathrm{i}\omega_0 (t+\varphi(\omega_0)/\omega_0)},\quad \tau_p(\omega_0)=-\frac{\varphi(\omega_0)}{\omega_0}$$
\end{example}
...
\begin{align*}
    ...
    \end{cases}
\end{align*}
其中，$x=0,y\neq 0$时的情况可以视为$\arctan(y/x)$的极限值，$\mathrm{Arg}$是连续的，但当$z$靠近$x$轴负半轴时，$\mathrm{Arg}(z)$的值趋于$\pi$或$-\pi$，产生幅度为$\pm 2\pi$的跳变。
...
在讨论群时延时我们将使用以下引理以简化讨论：

\begin{lemma}\label{lemma:cos_filter}
    设系统的单位脉冲响应$h(t)$是实信号，则当单频余弦函数$e(t)=\cos(\omega_0 t+\theta)$通过系统时，响应为
    $$r(t)=|H(\omega_0)|\cos(\omega_0 t+\theta+\varphi(\omega_0))$$
\end{lemma}
\begin{proof}
    由欧拉公式，
    $$e(t)=\cos(\omega_0 t+\theta)=\frac{\e^{\im(\omega_0 t+\theta)}+\e^{-\im(\omega_0 t+\theta)}}{2}$$
    因此其傅里叶变换为
    $$E(\omega)=\frac{\e^{\im\theta}\delta_{\omega_0}+\e^{-\im\theta}\delta_{-\omega_0}}{2}$$
    单位脉冲响应$h(t)$是实信号时，频率响应具有共轭对称性：
    $$H(-\omega)=\overline{H(\omega)},\quad \text{即}|H(-\omega)|=\left|H(\omega)\right|,\varphi(-\omega)=-\varphi(\omega)$$
    因此
    \begin{align*}
        R(\omega)&=H(\omega)E(\omega)\\
        &=\frac{H(\omega_0)\e^{\im\theta}\delta_{\omega_0}+H(-\omega_0)\e^{-\im\theta}\delta_{-\omega_0}}{2}\\
        &=|H(\omega_0)|\frac{\e^{\im(\theta+\varphi(\omega_0))}\delta_{\omega_0}+\e^{-\im(\theta+\varphi(\omega_0))}\delta_{-\omega_0}}{2}
    \end{align*}
    做傅里叶逆变换，得
    $$r(t)=|H(\omega_0)|\frac{\e^{\im(\omega_0 t+\theta+\varphi(\omega_0))}+\e^{-\im(\omega_0 t+\theta+\varphi(\omega_0))}}{2}=|H(\omega_0)|\cos(\omega_0 t+\theta+\varphi(\omega_0))$$
\end{proof}

\begin{example}[群时延]
    \label{exmp:group_delay}
    设输入信号为调幅信号（关于调幅信号，见\refsub{modulation}）：
    ...
    \end{align*}
    由于$\omega_c\gg \omega_m$，可以将$\left|H(\omega)\right|$近似为$|H(\omega_c)|$，将$\varphi(\omega)$在$\omega_c$处做泰勒展开：
    \begin{align*}
...
\begin{definition}[线性相位系统]
    如果系统对各个频率成分有相同时延，即相时延$\tau_p(\omega)$是常数，则称之为\textbf{线性相位系统}(linear phase system)；

    如果系统对各个中心频率的信号群有相同时延，即群时延$\tau_p(\omega)$是常数，则称之为\textbf{准线性相位系统}(quasilinear phase system).
...
为了讨论方便，我们假设波动是一维的，$x$处的质点在时刻$t$的位置为$y(x,t)$，$y(x,t)$满足的动力学方程称为\textbf{波动方程}，具有如下统一形式：
\[\frac{\partial^2 y}{\partial x^2}=\frac{1}{u^2}\frac{\partial^2 y}{\partial t^2}\]
其中常数$u$具有波速的物理意义。波动方程的解具有形式$y(x,t)=f(x-ut)+g(x+ut)$，其中$f$和$g$分别表示向x轴正方向和负方向传播的波的波形，数学上，它们可以是任意的无限阶可导函数。
...
这里波数并不是指波的数目，而是一个将空间距离折算为相位延迟的比例系数，也可以将它理解为波的空间角频率：对简谐波$y(x,t)=A\cos(\omega t-kx)$的空间变量做傅里叶变换，将得到空间角频率仅有$\pm k$的成分，对三维空间中的平面波也有类似的结论，见\refsub{multi_schwartz}。我们将宗量$\omega t-kx$称为波的\textbf{相位}(phase)，考察相位的传播速度，即令$\omega t-kx=\text{const}$（例如，振幅最大值处恒有$\omega t-kx=0$），得到
\begin{align*}
    \frac{dx}{dt}=\frac{\omega}{k}=u\tag{$*$}
\end{align*}
这个速度称为\textbf{相速度}(phase velocity)，记为$v_p$，它描述了波的相位传播的速度，可以看到相速度正是波速$u$。
...
\[H(\omega)=\mathrm{e}^{-\mathrm{i}\omega \frac{L}{u}}=\mathrm{e}^{-\mathrm{i}kL},\quad k=\frac{\omega}{u}\]
我们用到了波速$u$的定义式($*$)。可以看出，$k$实际上是关于$\omega$的函数$k(\omega)=\omega/u$，但在研究单频正弦波时看不出这一点。
...
\[H(\omega)=\mathrm{e}^{-\mathrm{i}k(\omega)L}\]
...
\begin{definition}[相速度和群速度]
    ...
    $v_p>v_g$时称为\textbf{正常色散}(normal dispersion)，前面介绍的波形展宽现象就是正常色散下的情况，而$v_p<v_g$时称为\textbf{反常色散}(anomalous dispersion)。
\end{definition}
...
电磁波是通过天线发射和接收的。在电路分析的课程中，我们学过：当实际电路的几何尺寸远小于使用时最高工作频率所对应的波长时，可以忽略电磁场相互作用，由此抽象出的理想化原件模型被称为\textbf{集总参数元件}(lumped element)；当实际电路的几何尺寸与使用时最高工作频率所对应的波长相当时，必须考虑电磁场相互作用，由此抽象出的理想化元件模型被称为\textbf{分布参数元件}(distributed element)。无线通信中的天线就是一个典型的分布参数元件，它将受金属束缚的电磁场（电压、电流）转化为辐射的电磁场，这个转化过程要求天线的尺寸必须与波长相当。以四分之一波长导体线为例，要发射1kHz的电磁波，则需要75km长的天线，这是难以实现的，因此无线通信通常使用MHz甚至GHz频率的电磁波作为载体。
...
\begin{definition}[调频、调幅、调相]
    ...
    \begin{itemize}
        \item 调幅信号为\[v_{\text{AM}}(t)=[1+k_{\text{AM}} m(t)]A_c \cos(\omega_c t+\varphi_c)\]其中$k_{\text{AM}}$为调幅灵敏度；
        \item 调频信号为\[v_{\text{FM}}(t)=A_c \cos\left(\omega_c t+k_{\text{FM}}\int_{0}^{t}m(\tau)\,d\tau+\varphi_c\right)\]其中$k_{\text{FM}}$为调频灵敏度；
        \item 调相信号为\[v_{\text{PM}}(t)=A_c \cos\left(\omega_c t+k_{\text{PM}} m(t)+\varphi_c\right)\]其中$k_{\text{PM}}$为调相灵敏度。
    \end{itemize}
\end{definition}
%也可以在载波的幅度和相位上同时调制，例如\textbf{正交振幅调制}(QAM, quadrature amplitude modulation)。

下面来解释为什么这三种调制方式的数学表示形式如上。\\
1.\textbf{幅度调制}：已调信号的幅度为\[1+k_{\text{AM}}m(t)\]
我们曾在\cref{exmp:group_delay}中使用这种调制方式来解释群时延的概念，事实上，当时所使用的是\textbf{双边带调幅}(\text{DSB})，一般而言不能略去1倍的载波信号这一项，即
\[v_{\text{AM}}(t)\neq k_{\text{AM}} m(t)A_c \cos(\omega_c t+\varphi_c)=v_{\text{DSB}}(t)\]
这是因为调幅信号通常通过包络检测器直接恢复，虽然有部分功率用于传输载波，但这简化了解调过程。此外，一般被调信号$m(t)$会预先经过衰减器以确保$|k_{\text{AM}}m(t)|\leq 1$，这样，$1+km(t)\geq 0$，包络不出现反转，即不会出现过度调制问题，包络检测器才能正常解调。\\
2.\textbf{频率调制}：$v_{\text{FM}}(t)$的傅里叶变换较为复杂，但其直观易于理解，这种调制方式下的\textbf{瞬时角频率}为：\begin{align*}
    \frac{d}{dt}\left(\omega_c t+k_{\text{FM}}\int_{0}^{t}m(\tau)\,d\tau+\varphi_c\right)=\omega_c+k_{\text{FM}}m(t)
\end{align*}
3.\textbf{相位调制}：\textbf{瞬时相位}为\[\omega_c t+k_{\text{PM}} m(t)+\varphi_c\]

我们以双边带调制为例给出一个简单的调制解调过程：
\begin{example}
设基带信号为$m(t)=\cos(\omega_m t)$，载波信号为$c(t)=\cos(\omega_c t)$，其中$\omega_c\gg \omega_m$。设计一个简单的解调器以还原出$m(t)$。\\
解：双边带调幅信号为\[v_{\text{DSB}}(t)=m(t)c(t)=\cos(\omega_m t)\cos(\omega_c t)\]
我们已经在上一节对它做过频域分析，得到其频谱为
\[V_{\text{DSB}}(\omega)=\frac{\pi}{2}\left[\delta_{\omega_c+\omega_m}+\delta_{-(\omega_c+\omega_m)}+\delta_{\omega_c-\omega_m}+\delta_{-(\omega_c-\omega_m)}\right]\]
假设传输过程没有发生失真，接收端得到的信号仍为$v_{\text{DSB}}(t)$。为了还原出$m(t)$，我们可以将$v_{\text{DSB}}(t)$与载波$c(t)$相乘，得到
\begin{align*}
    y(t)&=v_{\text{DSB}}(t)c(t)\\
...
我们将只含有低频成分（相对于载波频率而言）的模拟信号称为\textbf{模拟基带信号}，这种信号都可以采用上述方法进行传输，这是由于三角函数具有\textbf{频谱搬移}(spectrum shifting)的性质：\begin{align*}
...
\end{align*}
经过两次与载波相乘，信号的一半频谱回到原来的位置，另一半频谱被搬移到$2\omega_c$处，可见只要信号的带宽小于$\omega_c$，就可以通过选择合适的低通滤波器还原出基带信号。
