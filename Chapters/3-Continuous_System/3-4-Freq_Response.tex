...
相比输入信号
\[e(t)=\cos(\omega_m t)\cos(\omega_c t)\]
输出信号的幅度大约变为$|H(\omega_c)|$倍，载波信号的相位延迟了$\varphi(\omega_c)$，调制信号的相位延迟了$\omega_m\left.\dfrac{d\varphi(\omega)}{d\omega}\right|_{\omega=\omega_c}$，折合到时间上，就是群时延
...
有时，信号的信息是蕴含在其包络特性当中的，而载波信号则是为了方便信号传输，因此我们更关心系统对包络信号造成的时延，而不关心系统对载波信号造成的时延，这时群时延就显得尤为重要了。
