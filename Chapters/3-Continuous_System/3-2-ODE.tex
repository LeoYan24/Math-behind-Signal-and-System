...
对于特解，需要一定的配凑技巧，特别地，如果f为一个t的多项式与某个齐次解的乘积，则需要带入固定形式的特解求解其中的参数（以下默认P、Q为多项式）：\begin{itemize}
    ...
    \item 如果$f(t)=\mathrm{e}^{\beta t}$，$\beta$不是特征根，则特解$y_p(t)=c\e^{\beta t}$，c为常数；
...
完全解中，$t\to\infty$时保留下来的分量称为\textbf{稳态响应}(steady state response)，记为$y_{ss}(t)$，例如一个常数；$t\to\infty$时趋于0的分量为\textbf{暂态响应}(transient state response)，记为$y_{ts}(t)$，例如$\mathrm{e}^{-\lambda t}$。
\end{definition}
...
\begin{example}[三种响应划分方式的区别]
    ...
    特征方程$\lambda+2=0$的解为$\lambda=-2$，从而有齐次解（自由响应）$y_h(t)=Ce^{-2t}$（$C$为常数）及特解（强迫响应）$y_p(t)=5+\e^{-t}$，完全解为
    $$y(t)=y_h(t)+y_p(t)=Ce^{-2t}+5+\e^{-t}$$
    带入$y(0)=0$得$C+6=1,C=-5$，因此齐次解为$y_h(t)=-5\e^{-2t}$，完全解为
    \[y(t)=-5\e^{-2t}+5+\e^{-t}\]
    其中，稳态响应为$y_{ss}(t)=5$，暂态响应为$y_{ts}(t)=\e^{-t}-5\e^{-2t}$.

    求零输入响应时，应求齐次方程满足初始条件的解，即
    $$y_{zi}(0)=C=1\implies C=1,y_{zi}(t)=\mathrm{e}^{-2t}$$
    而求零状态响应时，则应该求原方程在初始条件为0时的解，即在完全解$y(t)=y_h(t)+y_p(t)=Ce^{-2t}+5+\e^{-t}$中，令$y(0)=C+6=0$得到$C=-6$，于是
...
\begin{example}
    ...
    则$$h'(t)=r_h'(t)u(t)+r_h(0)\delta+C\delta',\quad h''(t)=r_h''(t)u(t)+r_h'(0)\delta+r_h(0)\delta'+C\delta''$$
    带入原方程左侧得到：\begin{align*}
        \text{LHS}=&\left(r_h''(t)u(t)+r_h'(0)\delta+r_h(0)\delta'+C\delta''\right)\\
    ...
    对比$\delta$及其各阶导数的系数，得到：\begin{align*}
        ...
        \end{cases}
    \end{align*}
    所以$$C=1,\quad r_h(0)=-3,\quad r'_h(0)=8$$
    ...
    带入原方程左侧得到：\begin{align*}
        \text{LHS}=&\left(r_h''(t)u(t)+r_h'(0)\delta+r_h(0)\delta'\right)\\
...
\begin{theorem}
    ...
    的齐次解为$y_h(t)$，则这个带初值条件的微分方程的通解为
    \[r(t)=\int_{0}^{t}h(s)e(t-s)\,ds + y_h(t)\]
\end{theorem}
\begin{proof}
...
数学上，激励信号，即方程的非齐次项，往往是在全时域上都存在的，但是将方程视作一个系统时，认为激励在0时刻才开始施加是合理的。这是因为$t=0$时刻的初值条件就是由$t<0$时刻的激励引起的，有了起始条件就不再需要激励在$t<0$时刻的信息，这体现出\textbf{常系数线性微分方程所描述的系统具有马尔可夫性}。如果我们希望考虑全时域上的非齐次项$e(t)$，将$\int_{-\infty}^0 h(s)e(t-s)\,ds$也放在响应$r(t)$的公式中，则$y_h(t)$应根据系统在“$-\infty$时刻”的零状态来确定，它当然应该是全为0的，这样，通解为：
\begin{align*}
