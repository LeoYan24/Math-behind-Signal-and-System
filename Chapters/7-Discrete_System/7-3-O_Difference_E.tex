...
尽管$N>1$时，差分方程的解不仅依赖于$r_{zs}[0]$，还依赖于$r_{zs}[1-N]$，但我们还是认为系统有马尔可夫性，这与\ref{sub:discrete_system_overview}中提到的马尔可夫性并不矛盾——在\ref{sub:difference_equation}的最后我们指出，这些初始值相当于$n=0$时刻$r_{zs}$的$0$到$N-1$阶差分，并且给定这些初始值后，$n>0$时刻的响应的确不依赖于$n<0$时刻的激励。
