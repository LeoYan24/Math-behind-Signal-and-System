...
以上方程是由后向差分方程转化而来的差分方程，即后向差分方程。类似地，也可以定义前向差分方程，但前向差分方程所描述的系统可能不具备因果性。
...
对于$k$阶后向差分$\nabla^k y[0]$，可以将它展开为$y[-k],y[-k+1],\dots,y[0]$的线性组合，因此将这$N$个初始条件取为$n=0$时刻的前$N-1$阶后向差分，就相当于给出
...
尽管$N>1$时，差分方程的解不仅依赖于$r_{\zs}[0]$，还依赖于$r_{\zs}[1-N]$，但我们还是认为系统有马尔可夫性，这与\refsub{discrete_system_overview}中提到的马尔可夫性并不矛盾——在\refsub{difference_equation}的最后我们指出，这些初始值相当于$n=0$时刻$r_{\zs}$的$0$到$N-1$阶差分，并且给定这些初始值后，$n>0$时刻的响应的确不依赖于$n<0$时刻的激励。

沿着\refsub{fundamental_solution}的思路，我们尝试由单位阶跃响应来求解单位脉冲响应，从而简化计算。
...
\begin{proof}
    ...
    \begin{align*}
        \sum_{k=0}^{N}a_k h[n-k] &= \delta[n],\quad\sum_{k=0}^{N}a_k g[n-k]= u[n]
    \end{align*}
...
当然，作为洛朗级数，我们还是可以通过奇点判断收敛域。

单边z变换与双边z变换类似，可以参照\refsub{z_operation}给出单边z变换的线性性、移位性质、卷积定理等性质。本小节中，我们简要介绍与双边z变换不同的性质：
\begin{proposition}[单边z变换的移位性质]
    设$x[n]$的单边z变换为$X(z)$，则\begin{align*}
    \mathcal{Z}\{x[n-k]u[n]\}&=z^{-k}X(z)+\sum_{i=0}^{k-1}x[i-k]z^{-i}=z^{-k}\left[X(z)+\sum_{i=1}^{k}x[-i]z^{i}\right]\\
    \mathcal{Z}\{x[n+k]u[n]\}&=z^{k}X(z)-\sum_{i=-k}^{-1}x[i+k]z^{-i}=z^{k}\left[X(z)-\sum_{i=0}^{k-1}x[i]z^{-i}\right]
    \end{align*}
\end{proposition}
\begin{proof}
    \begin{align*}
...
首先，令$z=\mathrm{e}^{\mathrm{i}\Omega}$，并使$z\to 1$，即$\Omega\to 0$，可见终值由变换域中的零频成分决定。这与连续时间终值定理$\lim_{t\to\infty}f(t)=\lim_{s\to 0}sF(s)$一致，都体现了“\textbf{低频成分对应时域的稳态行为}”。

其次，上面的留数证明要求$X(z)$的奇点均为极点，类似的论证曾用于\refsub{causual_static_delay}。若存在其他类型的奇点，可仿照\refsub{initial_and_final_value}，将$X(z)$分解为$z=1$处的一阶分式与$|z|>r$上的全纯函数之和，从而得到相同结论。

最后，多数文献也允许$z=1$不是奇点。若$X(z)$的奇点都严格位于单位圆内，则其收敛域的内边界半径$r<1$。由洛朗级数的收敛半径公式，
\[\varlimsup_{n\to\infty}|x[n]|^{1/n}=r<1,\]
故$x[n]\to 0$，终值定理仍成立，其终值为零。反之，若奇点都位于单位圆外，则$r>1$，信号必然无界，且极限未必存在。更一般地，在已知收敛域方向的前提下，奇点到原点的距离决定信号的指数包络，奇点的辐角决定振荡频率，奇点的阶数则决定多项式包络。因此，与\refsub{converge_abscissa}类似，可以由$X(z)$的奇点判断$x[n]$的时域特性；这一结论也可由\refsub{laurant_series}中的洛朗级数理论严格说明。

\begin{example}
    考察单边z变换
    \[X_m(z)=\frac{1}{(1-az^{-1})^m}=\frac{z^m}{(z-a)^m},\qquad |z|>|a|,\]
    其中$a=\rho\mathrm{e}^{\mathrm{i}\Omega}\neq 0$。对$(1-az^{-1})^{-m}$做泰勒级数展开可得：
    \[x_m[n]=\binom{n+m-1}{m-1}a^n u[n]
    =\binom{n+m-1}{m-1}\rho^n\mathrm{e}^{\mathrm{i}\Omega n}u[n].\]
    $X_m(z)$仅在$z=a$处有一个$m$重极点。极点的模$\rho$控制指数包络：$\rho<1$时信号衰减，$\rho=1$时没有指数衰减或增长，$\rho>1$时信号增长；极点的辐角$\Omega$给出振荡频率；极点的阶数$m$产生$n^{m-1}$量级的多项式包络。例如，$m=1,a=1/2$时$x[n]=(1/2)^n u[n]$指数衰减，而$m=2,a=1/2$时$x[n]=(n+1)(1/2)^n u[n]$带有一次多项式包络。对于实信号，共轭极点成对出现，并合成为相应的正弦或余弦振荡。
\end{example}
...
利用上一小节中得到的移位性质，有\begin{align*}
    \sum_{k=0}^{N}a_k z^{-k}\left[Y(z)+\sum_{i=1}^{k}y[-i]z^{i}\right]&=\sum_{l=0}^{M}b_l z^{-l}\left[X(z)+\sum_{j=1}^{l}x[-j]z^{j}\right]
\end{align*}
其中，\begin{align*}
...
\end{align*}
因此，\begin{align*}
    Y(z)=\frac{\sum_{l=0}^{M}b_l z^{-l}\left[X(z)+\sum_{j=1}^{l}x[-j]z^{j}\right]-\sum_{k=0}^{N}a_k z^{-k}\sum_{i=1}^{k}y[-i]z^{i}}{\sum_{k=0}^{N}a_k z^{-k}}
\end{align*}

特别地，当方程右边恰为$x[n]$时，$M=0,b_0=1$，上式简化为\begin{align*}
    Y(z)=\frac{X(z)-\sum_{k=0}^{N}a_k z^{-k}\sum_{i=1}^{k}y[-i]z^{i}}{\sum_{k=0}^{N}a_k z^{-k}}
\end{align*}

\begin{example}
    求解差分方程
    $$y[n]-3y[n-1]+2y[n-2]=2^n$$
    在$n\in\mathbb{N}$时的解，初始值为$y[-2]=0,y[-1]=1$。

    解：将方程两边同时乘$u[n]$并做单边z变换，得到\begin{align*}
    Y(z)-3(z^{-1}Y(z)+y[-1])+2(z^{-2}Y(z)+y[-2]+y[-1]z^{-1})&=\frac{z}{z-2}
    \end{align*}
    代入并整理得\begin{align*}
    (1-3z^{-1}+2z^{-2})Y(z)&=\frac{z}{z-2}+3y[-1]-2y[-2]+2y[-1]z^{-1}=\frac{z}{z-2}+3-2z^{-1}
    \end{align*}
    因此\begin{align*}
    Y(z)=\frac{z}{(z-2)(1-3z^{-1}+2z^{-2})}+\frac{3+2z^{-1}}{1-3z^{-1}+2z^{-2}}=\frac{4z(z-1)}{(z-2)^2}
    \end{align*}
    由逆z变换的留数定理公式，选取$|z|>2$中的围道$C$，有\begin{align*}
    y[n]u[n]&=\frac{1}{2\pi\mathrm{i}}\oint_{C}Y(z)z^{n-1}\mathrm{d}z\\
    &=\mathrm{Res}\left(\frac{4z^n(z-1)}{(z-2)^2},2\right)\\
    &=\lim_{z\to 2}\frac{d}{dz}\left(4z^n(z-1)\right)\\
    &=4\lim_{z\to 2}\left(nz^{n-1}(z-1)+z^n\right)\\
    &=4\cdot 2^{n-1}(n+2)=2^{n+1}(n+2),\quad n\in\mathbb{N}
\end{align*}
\end{example}
