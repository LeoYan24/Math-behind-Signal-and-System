...
\begin{theorem}
    ...
    令
    \[\sigma_s=\sup\left\{\mathrm{Re}(s)_j\in\mathbb{C}\left|s_j\text{为}F(s)\text{的奇点}\right.\right\}\]
    则有$\sigma_c=\sigma_s$。
\end{theorem}
\begin{proof}
    一方面，$\sigma_c\geq\sigma_s$。$\mathcal{L} f(s)$在$\mathrm{Re}(s)<\sigma_s$上当然是发散的，因为$F(s)$在$\mathrm{Re}(s)=\sigma_s$上有奇点。

    另一方面，$\sigma_c\leq\sigma_s$。用反证法。假设$\sigma_c>\sigma_s$，则在带状区域$\sigma_s<\mathrm{Re}(s)\leq\sigma_c$上，$F(s)$满足定理5.3.2.的条件，它可以做拉普拉斯逆变换，得到唯一的时域函数，由于拉普拉斯逆变换
...
因此我们可以\textbf{通过分析拉普拉斯变换的奇点来确定$f(t)$的增长速率}（的上确界），也可以\textbf{通过分析$f(t)$的增长速率来确定拉普拉斯变换的奇点位置}。
...
\begin{definition}[系统函数]
    ...
    如果虚轴$\mathrm{Re}(s)=0$在$H(s)$的收敛域内，则$H(\mathrm{i}\omega)$与第四章中定义的频率响应一致。
\end{definition}
