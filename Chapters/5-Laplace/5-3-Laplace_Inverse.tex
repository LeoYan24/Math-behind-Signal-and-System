...
\begin{remark}
    ...
    应该理解为沿着复平面上的直线$\mathrm{Re}\ z=b$从$b-\mathrm{i}\infty$到$b+\mathrm{i}\infty$的复线积分。由于全纯函数的路径无关性，我们也可以选择其他路径，甚至可以添加一些路径，再使用留数定理计算拉普拉斯逆变换，见本节后续的讨论。
\end{remark}
\begin{proof}
    \begin{align*}
...
但是，以上证明已经假定了$F(s)=\mathcal{L} f(s)$是某个函数$f\in\mathcal{E} $的拉普拉斯变换。从另一个角度出发，假设有一个在右半平面$\mathrm{Re}\ s>a$上全纯的函数$F(s)$，就需要考虑它是否是某个函数的拉普拉斯变换，以及如何求出这个函数。为此，我们给出以下定理\cite{distribution}：
\begin{theorem}[*拉普拉斯反演公式]
...
本小节介绍使用留数定理计算拉普拉斯逆变换的方法。一般地，我们选择由$b\pm \mathrm{i}r,-r\pm \mathrm{i}r$组成的矩形围道，其中$b>0$需保证$F(s)$在$\mathrm{Re}\ s>b$上全纯（因此在$r\to\infty$时极点全部位于围道内），见下方左图：
\begin{figure}[H]
    \centering
...
\begin{example}[真分式函数，留数定理方法]
    ...
    \begin{align*}
        \left|\int_{top}F(s)\mathrm{e}^{st}\mathrm{i}\,ds\right|&\leq \sup_{x\leq b}|F(x+\mathrm{i}r)\mathrm{e}^{(x+\mathrm{i}r)t}|\int_{-\infty}^{b}\mathrm{e}^{xt}\,dx\\
        &\leq \frac{\mathrm{e}^{bt}}{t}\sup_{x\leq b}|F(x+\mathrm{i}r)|
    \end{align*}
    在前面的引理中，我们已经证明了$|F(x+\mathrm{i}r)|\to 0,|r|\to\infty$，因此上边的积分趋于0。对于矩形的左边，$|\mathrm{e}^{st}|=\mathrm{e}^{-rt}$，以指数函数的速度衰减至0，积分当然趋于0。

    当$t<0$时，$\mathrm{e}^{st}$在$\mathrm{Re}\ s$充分大时有较小的模值，因此我们可以使用上面右图中的围道来论证$f(t)=0$。而$t=0$的值则不需要考虑。
    
    根据留数定理，有
    \begin{align*}
        f(t)&=\sum \mathrm{Res}[F(s)\mathrm{e}^{st},s_k]\\
        &=\mathrm{Res}\left[\frac{2s+5}{(s+1)(s+2)}\mathrm{e}^{st},-1\right]+\mathrm{Res}\left[\frac{2s+5}{(s+1)(s+2)}\mathrm{e}^{st},-2\right]\\
...
\begin{example}*
    ...
    \begin{align*}
        \mathrm{Res}[F(s)\mathrm{e}^{st},\mathrm{i}n]&=\lim_{s\to \mathrm{i}n}(s-\mathrm{i}n)F(s)\mathrm{e}^{st}\\
