...
在\refsub{introduction_of_fourier}中，我们介绍了傅里叶变换：\begin{align*}
    \mathcal{F} f(\omega)=\int_{-\infty}^{\infty}f(t)\mathrm{e}^{-\mathrm{i}\omega t}\,dt
\end{align*}

在不考虑收敛性的问题时，可以将$\omega$推广为复数$s=\sigma+\mathrm{i}\omega$，根据含参变量积分的求导法则，这样的积分变换可以对$s$求导，从而是全纯函数。这个积分变换就是\textbf{双边拉普拉斯变换}：
\begin{definition}[双边拉普拉斯变换]
    \label{def:bi_laplace}
    设函数$f$局部可积，$\exists a,b\in\mathbb{R},a<b$（我们允许$a=-\infty$和$b=\infty$），使得$f(t)\mathrm{e}^{-at}$和$f(t)\mathrm{e}^{-bt}$均在$\mathbb{R}$上可积，$f$的双边拉普拉斯变换定义为
    \[\mathcal{L}f(s)=\int_{-\infty}^{\infty}f(t)\mathrm{e}^{-st}\,dt,\quad s=\sigma+\mathrm{i}\omega,a\le \sigma\le b\]
...
在$\sigma>0$时，这个收敛因子会使得$t\to -\infty$时$\left|\mathrm{e}^{-st}\right|=\mathrm{e}^{-\sigma t}$快速增长，从而对被变换的函数$f$提出很高的要求。事实上，多数情况下$f$支于$[a,\infty),a\in\mathbb{R}$为常数，问题化归为因果信号（\cref{def:casual_signal}）的拉普拉斯变换，即\textbf{单边拉普拉斯变换}\cite{folland}：
\begin{definition}[单边拉普拉斯变换]
    \label{def:single_laplace}
    记\[\mathcal{E}=\left\{f:[0,\infty)\to\mathbb{C}\left|f\in PC[0,\infty),\exists C>0,a\in\mathbb{R},|f(t)|<C\e^{at},\forall t\geq 0\right.\right\}\]
    其中$PC[0,\infty)$表示$[0,\infty)$上的分段连续函数。
    对$f\in\mathcal{E}$，其单边拉普拉斯变换定义为
    \[\mathcal{L} f(s)=\int_{0}^{\infty}f(t)\mathrm{e}^{-st}\,dt,\quad s=\sigma+\mathrm{i}\omega,\sigma>a\]
\end{definition}
在$\mathcal{E} $的定义中我们对$f$的定义域做了要求，但实际上只要$f$是因果信号，就可以认为$f\in\mathcal{E}$，因此后文中不会特意区分$f$的定义域。

下面几节中，我们将介绍单边拉普拉斯变换的性质，它们大多容易推广到双边拉普拉斯变换的情形。如无特殊说明，本节后文中所说的拉普拉斯变换均指单边拉普拉斯变换。

在$\mathcal{E} $上定义拉普拉斯变换，是因为对于$f\in\mathcal{E} $，当$\sigma>a$时，给出拉普拉斯变换的积分是绝对收敛的：
\begin{align*}
    \int_0^{\infty}|f(t)\mathrm{e}^{-st}|\,dt\leq \int_0^{\infty}C\e^{at}\mathrm{e}^{-\sigma t}\,dt=\frac{C}{\sigma-a}<\infty
\end{align*}
...
\begin{remark}
    其实，这个公式对实部大于$-1$的$\nu$都成立。尽管$-1<\mathrm{Re}(\nu)<0$时，$f(t)=t^\nu$\\
    在$t=0$处无界，但由于
\[\left|\int_{0}^{1}t^{\nu}\mathrm{e}^{-st}\,dt\right|\leq \int_0^1 t^\nu\,dt=\frac{1}{\nu+1},\quad \sigma>0\]
\[\left|\int_{1}^{\infty}t^{\nu}\mathrm{e}^{-st}\,dt\right|\leq \int_1^\infty \mathrm{e}^{-\sigma t}\,dt=\frac{\mathrm{e}^{-\sigma}}{\sigma},\quad \sigma>0\]
其拉普拉斯变换是存在的。因此，我们可以认为$t^\nu\in\mathcal{E},\mathrm{Re}(\nu)>-1$.
\end{remark}
\end{example}
...
\begin{proposition}[$\mathcal{E} $中的卷积]
    ...
    \[(f*g)(t)=\begin{cases}
    \displaystyle 0                   ,& \text{if } t<0 \\
    \displaystyle\int_0^t f(x)g(t-x)\,dx ,& \text{if } t\geq 0
    \end{cases}\]
\end{proposition}
这是因为$x\geq 0,t-x\geq 0$时，$f(x),g(t-x)$才不恒为0，这相当于积分区间为$[0,t]$。

\subsection{单边拉普拉斯变换的运算性质}\label{sub:laplace_operation}
...
\begin{proposition}[拉普拉斯变换的性质]
    设$f,g\in\mathcal{E} $，$a,b\in\mathbb{C}$，则：\begin{itemize}
        ...
        \item 时移：\[\mathcal{L} [f(t-t_0)u(t)](s)=\mathrm{e}^{-s t_0}\mathcal{L} f(s)\]
        ...
        此处积分路径为复平面上满足$\mathrm{Im}(s)=\omega$有界的任意从$s$到无穷远处的路径
    \end{itemize}
\end{proposition}
\begin{remark}
...
\begin{proof}
    1.线性性：直接得自积分的线性性。\\
    2.时移：\begin{align*}
        \mathcal{L} [f(t-t_0)u(t)](s)&=\int_0^{\infty}f(t-t_0)\mathrm{e}^{-st}\,dt\\
    ...
    4.伸缩：\begin{align*}
        ...
        &=\int_0^{\infty}f(x)\mathrm{e}^{-sx/a}\,\frac{dx}{a}&(x=at)\\
...
\subsection{常用函数的单边拉普拉斯变换}\label{sub:ordinary_laplace}

\begin{example}[指数多项式]
    设$f(t)=t^\nu \mathrm{e}^{a t}u(t),\mathrm{Re}(\nu)>-1$，利用拉普拉斯变换的频移性质，有
    \begin{align*}
...
\begin{example}[正弦、余弦函数]
    ...
    \begin{align*}
        ...
        &=\frac{s}{s^2+\omega_0^2},\quad \sigma>0
    \end{align*}
    \begin{align*}
        ...
        &=\frac{\omega_0}{s^2+\omega_0^2},\quad \sigma>0
    \end{align*}
    方法二：利用微分性质：
    \begin{align*}
    ...
    \end{align*}
    而$\cos(\omega_0 t)$具有常数级的增长速度（在\refsub{converge_abscissa}中，我们将定量描述“增长速度”这一概念），因此
    \[\mathcal{L} [\cos(\omega_0 t)](s)=\frac{s}{s^2+\omega_0^2},\quad \sigma>0\]
    同理，有\[\mathcal{L} [\sin(\omega_0 t)](s)=\frac{\omega_0}{s^2+\omega_0^2},\quad \sigma>0\]
\end{example}

\begin{example}[双曲函数]
    因为
    \[\sinh(\omega_0 t)=\frac{\mathrm{e}^{\omega_0 t}-\mathrm{e}^{-\omega_0 t}}{2},\quad \cosh(\omega_0 t)=\frac{\mathrm{e}^{\omega_0 t}+\mathrm{e}^{-\omega_0 t}}{2}\]
    所以
    \begin{align*}
        ...
        &=\frac{\omega_0}{s^2-\omega_0^2},\quad \sigma>|\omega_0|
    \end{align*}
    \begin{align*}
        ...
        &=\frac{1}{2}\left(\frac{1}{s-\omega_0}+\frac{1}{s+\omega_0}\right)\\
        &=\frac{s}{s^2-\omega_0^2},\quad \sigma>|\omega_0|
    \end{align*}
\end{example}
...
\begin{example}[采样函数]
    ...
    \begin{align*}
        ...
        &=\arctan\left(\frac{\omega_0}{s}\right),\quad \sigma>0
    \end{align*}
\end{example}

\begin{example}[正弦积分函数]
    Si函数定义见\refsub{int_sin}.
    $$\frac{\sin(\omega_0 t)}{t}\leq C\e^{a t},a>0\implies \mathrm{Si}(\omega_0 t)\leq \frac{C}{\omega_0 a}\mathrm{e}^{a t}\implies \mathrm{Si}(\omega_0 t)\in\mathcal{E}$$
    利用正弦函数的拉普拉斯变换和拉普拉斯变换的时域积分性质，有
    \begin{align*}
        \mathcal{L} [\mathrm{Si}(\omega_0 t)](s)&=\mathcal{L} \left[\int_0^{\omega_0 t} \frac{\sin(x)}{x}\,dx\right](s)\\
        &=\mathcal{L} \left[\int_0^{t} \frac{\sin(\omega_0 x)}{x}\,dx\right](s)\\
        &=\frac{1}{s}\mathcal{L} \left[\frac{\sin(\omega_0 t)}{t}\right](s)\\
        &=\frac{1}{s}\arctan\left(\frac{\omega_0}{s}\right),\quad \sigma>0
    \end{align*}    
\end{example}

\subsection{*单边拉普拉斯变换的级数求法}\label{sub:laplace_by_series}
...
\begin{lemma}
    设$f\in\mathcal{E} ,|f(t)|\leq C\e^{at}$，则\begin{enumerate}
        \item $\forall x>a,\mathcal{L} f(x+\mathrm{i}y)\to 0,|y|\to\infty$
        \item $\forall y,\mathcal{L} f(x+\mathrm{i}y)\to 0,x\to\infty$
    \end{enumerate}
\end{lemma}
\begin{proof}
    1.令$g(t)=f(t)\mathrm{e}^{-x t}$，则$|g(t)|\leq C\e^{-(x-a)t}$，因此$g\in L^1(\mathbb{R} )$，根据Riemann-Lebesgue引理，有
    \[\mathcal{L} f(x+\mathrm{i}y)=\mathcal{F} g(y)\to 0,\quad |y|\to\infty\]
    2.利用海涅归并原理，任取实部趋于无穷的序列$\{z_n\}_{n=1}^\infty$，满足$\mathcal{L} f(z_n)$存在（这要求$\mathrm{Re}(z)>a$），我们要证明序列$\mathcal{L} f(z_n)\to 0$.
    $$\int_{0}^{\infty}\left|f(t)\mathrm{e}^{-z_n t}\right|\,dt\leq \int_0^{\infty}C\e^{at}\mathrm{e}^{-\mathrm{Re}(z_n) t}\,dt=C\int_0^{\infty}\mathrm{e}^{-(\mathrm{Re}(z_n)-a)t}\,dt=\frac{C}{\mathrm{Re}(z_n)-a}<\infty$$
    由控制收敛定理（\cref{thm:DCT}），取$\left|f(t)\mathrm{e}^{-z_1 t}\right|$作为控制函数，得到
    \begin{align*}
...
\end{theorem}
\begin{proof}
    由条件知级数$\sum_{n=0}^{\infty}a_n \dfrac{\Gamma(n+1)}{s^{n+1}}$在$\sigma>\omega_0$时绝对收敛、内闭一致收敛，因此只需证明其和函数等于$\mathcal{L} f(s)$。拉普拉斯变换是一种积分变换，我们需要将求和与积分交换，可以考虑使用控制收敛定理。由于
    \begin{align*}
        &|a_n \Gamma(n+1)\omega_0^{-n}|\to 0, n\to\infty\\
        \implies &|a_n|\leq C\frac{\omega_0^{n}}{\Gamma(n+1)}\\
        \implies &|f(t)|\leq \sum_{n=0}^{\infty}|a_n|t^n\leq C\sum_{n=0}^{\infty}\frac{(\omega_0 t)^{n}}{\Gamma(n+1)}=C\e^{\omega_0 t}\\
        \implies &\forall s,\mathrm{Re}(s)=\omega_0+\phi>\omega_0,|f(t)\mathrm{e}^{-s t}|\leq C\e^{-\phi t}
    \end{align*}
    控制收敛定理的条件成立，换序即证。
\end{proof}
...
\begin{corollary}*
   ...
   其中$0\leq\alpha<1$为常数，如果存在$\omega_0>0$，使得级数
