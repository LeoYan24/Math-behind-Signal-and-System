...
在$\sigma>0$时，这个收敛因子会使得$t\to -\infty$时$\left|\mathrm{e}^{-st}\right|=\mathrm{e}^{-\sigma t}$快速增长，从而对所变换的函数提出很高的要求。实际上，更多时候我们仅使用\textbf{单边拉普拉斯变换}，定义如下\cite{folland}：
...
\end{definition}
在$\mathcal{E} $的定义中我们对$f$的定义域做了要求，但实际上只要$f$是因果信号（\cref{def:casual_signal}），就可以认为$f\in\mathcal{E}$，因此后文中不会特意区分$f$的定义域。
...
\begin{example}[正弦、余弦函数]
    首先，$\cos(\omega_0 t),\sin(\omega_0 t)\in\mathcal{E}$，因为$$\forall a>0,\quad |\cos(\omega_0 t)|,|\sin(\omega_0 t)|<\mathrm{e}^{a t}$$
    下面给出两种求解方法。\\
    方法一：利用欧拉公式化归为指数函数：
    \begin{align*}
