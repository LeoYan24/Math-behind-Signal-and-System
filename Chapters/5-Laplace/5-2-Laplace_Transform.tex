...
\end{definition}
在$\mathcal{E} $的定义中我们对$f$的定义域做了要求，但实际上只要$f$是因果信号（\cref{def:casual_signal}），就可以认为$f\in\mathcal{E}$，后文中不会特意区分$f$的定义域。
...
\begin{example}[正弦积分函数]
    $$\frac{\sin(\omega_0 t)}{t}\leq Ce^{a t},a>0\implies \mathrm{Si}(\omega_0 t)\leq \frac{C}{a}\mathrm{e}^{a t}\implies \mathrm{Si}(\omega_0 t)\in\mathcal{E}$$
...
\begin{proof}
    ...
    2.利用海涅归并原理，任取实部趋于无穷的序列$\{z_n\}$，满足$\mathcal{L} f(z_n)$存在（这要求$\mathrm{Re}(z)>a$），我们要证明序列$\mathcal{L} f(z_n)\to 0$。由控制收敛定理（\cref{thm:DCT}），有
    \begin{align*}
        &\int_{0}^{\infty}|f(t)\mathrm{e}^{-z_n t}|\,dt\leq \int_0^{\infty}Ce^{at}\mathrm{e}^{-Re(z_n) t}\,dt=C\int_0^{\infty}\mathrm{e}^{-(Re(z_n)-a)t}\,dt=\frac{C}{\mathrm{Re}(z_n)-a}<\infty\\
