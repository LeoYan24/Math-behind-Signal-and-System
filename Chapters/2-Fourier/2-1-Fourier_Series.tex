...
读者在学习本节之前，最好在数学分析等课程中已经对傅里叶级数有所了解。
...
\end{align*}
其中$\omega =\dfrac{2\pi }{T}$为\textbf{基波角频率}(fundamental angular frequency)，$k\omega (k>1,k\in \mathbb{N} )$为k次\textbf{谐波角频率}(harmonic angular frequency)。
\begin{theorem}[三角函数形式的傅里叶系数公式]
...
可以将它们单位化，也可以直接采用正交基分解的公式（\cref{thm:vector_decomposition}）：
\[a_k=\frac{(f(t),\cos(k\omega t))}{(\cos(k\omega t),\cos(k\omega t))}=\frac{\int_{T}f(t)\overline{\cos(k\omega t)}\,dt}{\int_{T}|\cos(k\omega t)|^2\,dt}
    =\frac{2}{T}\int_{T}f(t)\cos(k\omega t)\,dt\]
\[b_k=\frac{(f(t),\sin(k\omega t))}{(\sin(k\omega t),\sin(k\omega t))}=\frac{\int_{T}f(t)\overline{\sin(k\omega t)}\,dt}{\int_{T}|\sin(k\omega t)|^2\,dt}
    =\frac{2}{T}\int_{T}f(t)\sin(k\omega t)\,dt\]
不难发现，如果将常数项写为$a_0/2$的形式，则$k=0$时上述公式仍然适用，这也是我们这样做的原因。
...
\end{align*}
则前半部分为偶函数，称之为$f(t)$的\textbf{偶分量}$f_\e(t)$；后半部分为奇函数，称之为$f(t)$的\textbf{奇分量}$f_\text{o}(t)$.函数的偶分量和奇分量都是唯一的，并且\begin{align*}
    f_\e(t)=\frac{f(t)+f(-t)}{2} \\
    f_\text{o}(t)=\frac{f(t)-f(-t)}{2}
\end{align*}

容易利用对称性得到：
\begin{theorem}[偶函数的傅里叶展开]
...
\begin{definition}[奇谐函数]
    函数$f(t)$称为\textbf{奇谐函数}(quasiharmonic function)，如果后半个周期的函数是前半个周期的负镜像，即\begin{equation*}
...
类似\refsub{chebychev_polynomial}中对于三角多项式的处理，对于三角级数，我们同样可以代入：
\[\cos(k\omega t)=\frac{\mathrm{e}^{\mathrm{i}k\omega t}+\mathrm{e}^{-\mathrm{i}k\omega t}}{2},\quad \sin(k\omega t)=\frac{\mathrm{e}^{\mathrm{i}k\omega t}-\mathrm{e}^{-\mathrm{i}k\omega t}}{2\mathrm{i}}\]

从而得到复指数形式的傅里叶级数：

\begin{definition}[复指数形式傅里叶展开]
    \[f(t)=\sum_{k = -\infty}^{\infty}  c_k \mathrm{e}^{\mathrm{i}k\omega t} ,\quad c_k=\frac{a_k-\mathrm{i}b_k}{2},c_{-k}=\frac{a_k+\mathrm{i}b_k}{2}=\overline{c_k},\quad k\in \mathbb{N}\]
特别地，$c_0=\overline{c_0}\implies c_0\in\mathbb{R} $.
\end{definition}
我们又一次使用了符号$c_k$，请注意它与本小节开头给出的三角级数中的符号$c_k$的区别。本书的后续内容中，$c_k$总是表示复指数级数的系数。

容易验证$\{\mathrm{e}^{\mathrm{i}k\omega t}\}_{k=0}^{\infty}$是正交函数系：
\begin{align*}
    ...
    &=0\quad (k_1,k_2\in \mathbb{Z},k_1\neq k_2)
\end{align*}
\begin{align*}
    (\mathrm{e}^{\mathrm{i}k\omega t},\mathrm{e}^{\mathrm{i}k\omega t})&=\int_{T}\mathrm{e}^{\mathrm{i}k\omega t}\overline{\mathrm{e}^{\mathrm{i}k\omega t}}\,dt\\
    &=\int_{T}\,dt\\
    &=T\quad (k\in \mathbb{Z} )
\end{align*}
和三角函数系的情况一样，可以由\cref{thm:vector_decomposition}得到$c_k$的计算公式：
\begin{align*}
    c_k & =\frac{(f(t),\mathrm{e}^{\mathrm{i}k\omega t})}{(\mathrm{e}^{\mathrm{i}k\omega t},\mathrm{e}^{\mathrm{i}k\omega t})} \\
        & =\frac{1}{T}\int_{T}f(t)\overline{\mathrm{e}^{\mathrm{i}k\omega t}}\,dt                 \\
        & =\frac{1}{T}\int_{T}f(t)\mathrm{e}^{-\mathrm{i}k\omega t}\,dt
\end{align*}
有时也将$c_k$记作$\hat{f}(k\omega)$或$\hat{F}(k)$，表示f在频域中的点$k\omega$处的值。一般而言，我们只将最小正周期称为一个函数的周期，但\textbf{周期为T的函数可以有多个频率$k\omega(k \in \mathbb{Z})$}。
...
对于实信号，
\[\hat{f}(k\omega)=\overline{\hat{f}(-k\omega)}\implies |\hat{f}(k\omega)|=|\hat{f}(-k\omega)|,\phi_{-k}=-\phi_k\]
...
\begin{align*}
    ...
    &=\frac{c_k}{2}\mathrm{e}^{\mathrm{i}(k\omega t+\varphi_k)}+\frac{\overline{c_{-k}}}{2}\mathrm{e}^{\mathrm{i}(-k\omega t+\varphi_{-k})}\\
    \implies c_k&=2\hat{f}(k\omega),\quad k>0
\end{align*}
...
\end{figure}
如图，可以看到，这个频谱与采样函数$\text{Sa}(\omega)$非常相似\footnote{为了体现这一点，绘制频谱时将基波角频率大幅减小，并不是第一张图直接做傅里叶级数展开的结果}，其原理将在\refsub{fourier_of_dirac_comb}中给出。
...
\begin{definition}[依范数收敛]
    ...
    其中
    \[\left\|f\right\|_2^2=(f,f)=\int_{0}^{T}|f(t)|^2\,dt,\quad\forall f\in L^2([0,T])\]
\end{definition}

作为一个引理，我们首先来证明（指数形式的）傅里叶级数是使用正交基$\{\mathrm{e}^{\mathrm{i}k\omega t}\}_{k=0}^{\infty}$给出的，\textbf{最小均方误差}(minimun mean sqare error,MMSE)意义下的的最佳逼近：
\begin{lemma}
...
\end{lemma}
\begin{proof}
    利用正交基的定义，$c_k$是函数$f$在基函数$\mathrm{e}^{\mathrm{i}k\omega t}$上投影分量的大小，因此去掉$c_k\e^{\im k\omega t}$成分后，$f$与$\e^{\im k\omega t}$是正交的：
    \[\left(f-\sum_{k=-N}^{N}c_k \mathrm{e}^{\mathrm{i}k\omega t},\mathrm{e}^{\mathrm{i}k\omega t}\right)=0,\quad k=-N,\cdots,N\]
    将下式与自己做内积：
...
为了证明依范数收敛性，我们还需要说明：三角多项式（\cref{def:trinomial}）在$L^2([0,T])$中是稠密的，这一结论可以拆分为以下两个命题。
\begin{theorem}[魏尔斯特拉斯第二逼近定理]
    在$L^2$范数的意义下，三角多项式（即形如傅里叶级数部分和的函数）在连续函数空间$C([0,T])$中是稠密的，即$\forall f\in C([0,T]),\forall \varepsilon >0$，存在一个三角多项式$P(t)$使得$\|f-P\|_2<\varepsilon$.
\end{theorem}
以上定理改写自\cref{cor:weierstrass_2}，证明也将在那里给出。

\begin{claim}
    连续函数空间$C([0,T])$在$L^2([0,T])$中是稠密的，即：
    $$\forall f\in L^2([0,T]),\forall\varepsilon >0,\exists g\in C([0,T]),\|f-g\|_2<\varepsilon$$
\end{claim}

由于傅里叶级数是$f$的最佳逼近，只要指出一个$M$次的三角多项式，使得$\|f-P\|_2<\varepsilon$，就有
\begin{align*}
    \|f-S_N\|_2 & \leq \|f-P\|_2<\epsilon,\quad\forall N>M
\end{align*}
综上，我们证明了：
\begin{theorem}[傅里叶级数的依范数收敛性]
    设$f\in L^2([0,T])$，指数形式的傅里叶系数为$c_k(k\in\mathbb{Z} )$，则傅里叶级数的部分和依范数收敛于$f$：
    \[\lim_{N \to \infty} \left\| f(t)-\sum_{k=-N}^{N}c_k \mathrm{e}^{\mathrm{i}k\omega t}\right\|_2=0\]
\end{theorem}

任意$L^2([0,T])$中的函数$f$均可展开为傅里叶级数，而傅里叶级数一定依范数收敛于$f$，即$f$作为一个矢量总可以表示为基函数的线性组合，因此我们说：复指数函数系$\{\e^{\im k\omega t}\}_{k=0}^{\infty}$和三角函数系是\textbf{完备}的正交基。
...
\begin{theorem}[狄利克雷定理]
    ...
    \[\forall t\in \mathbb{R},\quad\sum_{k=\infty}^{\infty}c_k \mathrm{e}^{\mathrm{i}2\pi kt}=\frac{f(t+)+f(t-)}{2}\]
\end{theorem}
...
狄利克雷条件中的最后一条实际上相当于要求f是有界变差函数，感兴趣的读者可以参考实变函数的教材（如\cite{simple_real_analysis}，\cite{real_analysis}）。在\refsub{uniform_converge}中我们将讨论另外的更易理解的条件：
\begin{definition}[分段光滑]
    \phantomsection\label{def:piecewice_smooth}
...
此外，逐点收敛性可以推广为点态的\textbf{Cesàro收敛}或\textbf{Abel收敛}。逐点收敛性强于这两种收敛性，一个数项级数收敛于某个值，则它必然Cesàro收敛、Abel收敛到同样的值。

事实上，只要函数$f\in L^1([0,T])\cap L^2([0,T])$，就能保证$f$的傅里叶级数在$f$的连续点处Cesàro收敛于$f(t)$，而Cesàro收敛又能推出Abel收敛。这一条件比狄利克雷条件弱一些。对于这两种收敛性的定义及其证明，可以参考\cite{stein2011fourier}。
...
为了直观地理解上确界的极限与极限的上确界之间的区别，我们来看一个经典的例子：
\begin{example}
    \begin{align*}
    f_n(x)=
    \begin{cases}
...
其中P为平均功率。
\end{theorem}
至此，我们提出了傅里叶系数的公式和帕塞瓦尔定理，但用到的毕达哥拉斯定理（\cref{thm:gougu}）是基于有限维线性空间的，现在就来填补这个逻辑漏洞，对此不感兴趣的读者可以忽略这部分内容。设$\{\phi_n\}_{n=1}^{\infty}$是内积空间$L^2((a,b))$的标准正交基，$f\in L^2((a,b))$（注意这里已经不局限于讨论傅里叶级数）。
...
\begin{flalign*}
     ...
     &=\|f\|_2^2-2\text{Re}\left(\left(f,\sum_{n=1}^{N}(f,\phi_n)\phi_n\right)\right)+\left\|\sum_{n=1}^{N}(f,\phi_n)\phi_n\right\|_2^2                        \\
     &=\|f\|_2^2-2\sum_{n=1}^{N}|(f,\phi_n)|^2+\sum_{n=1}^{N}|(f,\phi_n)|^2\\
     &=\|f\|_2^2-\sum_{n=1}^{N}|(f,\phi_n)|^2
\end{flalign*}
令$N\to\infty$即证。从第一行到第二行用到了恒等式$\|\mathbf{a+b}\|^2=\|\mathbf{a}\|^2+2\text{Re}(\langle\mathbf{a,b}\rangle)+\|\mathbf{b}\|^2$，这个结论十分简单，留予读者自证。
\end{proof}
在最终的结论帕塞瓦尔定理中这个不等号将变成等号，但它是不可或缺的，并且我们还将在\cref{lemma:bessel_inequality}中见到它。
...
\begin{proof}
    由贝塞尔不等式，
\begin{flalign*}
    \sum_{n=1}^{\infty}|(f,\phi_n)|^2\leq\|f\|_2^2<\infty
\end{flalign*}
因此
$$\lim_{n\to\infty}|(f,\phi_n)|\to 0$$
任取$m_1,m_2\in\mathbb{N},m_1<m_2$，由勾股定理，
\begin{flalign*}
              \left\|\sum_{n=m_1}^{m_2}(f,\phi_n)\phi_n\right\|_2^2=\sum_{n=m_1}^{m_2}\left|(f,\phi_n)\right|^2\to 0
\end{flalign*}
因此$\sum_{n=1}^{\infty}(f,\phi_n)\phi_n$构成柯西列。由于$L^2((a,b))$是无限维的完备度量空间，即希尔伯特空间（
\cref{def:hilbert_space}）。
...
\begin{proposition}[*希尔伯特空间中的内积的连续性]\label{prop:hilbert_inner_product_continuous}
...
\begin{theorem}*
以下三个命题是等价的：（符号$\sim$表示$L^2((a,b))$空间中的等价关系，参考\refsub{Lp_space}）
\begin{enumerate}
...
$1\implies 2$：令$g\sim \left(f-\sum_{n=1}^{\infty}(f,\phi_n)\phi_n\right)$，则$\forall m\in\mathbb{N}$，有：\begin{align*}
    \left(g,\phi_m\right)=\left(f,\phi_m\right)-\sum_{n=1}^{\infty}\left(f,\phi_n\right)\left(\phi_n,\phi_m\right)=\left(f,\phi_m\right)-\left(f,\phi_m\right)=0 
\end{align*}
根据1知g=0，即2.这里内积与求和的换序是由\cref{prop:hilbert_inner_product_continuous}保证的。\\
$2\implies 3$：由勾股定理，
