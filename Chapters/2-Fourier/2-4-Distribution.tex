...
取$g=f$即证。
\end{proof}
相比\ref{sub:fourier_operation_properties}中的帕塞瓦尔定理，这个证明适用于$f$是正则分布而$g$是施瓦兹函数的情况，它的证明要简单得多。
...
因此\[f*\delta_a=\tau_a f\]
\end{example}
在\ref{sub:fourier_of_signal}中，我们已经从形式上得到了$\mathrm{sgn},u,1/t$，的傅里叶变换，现在有了分布的工具，就可以借助分布的傅里叶变换的对偶性说明之前的结论是严谨的。
...
分布的傅里叶变换的连续性是在弱收敛的意义下定义的，也就是说，作为一个分布，我们总是能够交换极限或求和与傅里叶变换的顺序（我们将会多次使用到这一结论），但是对于正则分布列$f_n\to f$，尽管$T_{f_n}\to T_f$，也不能认为$\mathcal{F}f_n$作为一个函数列收敛于$\mathcal{F}f$。在\ref{sec:Sampling_Interpolation}中我们将看到，通过分布的傅里叶变换的连续性，$\shah$分布能表示成傅里叶级数的形式，然而这个展开式是几乎处处不收敛的函数项级数。

事实上，施瓦兹函数的傅里叶变换也是具有连续性的。然而，证明它要困难得多，我们将证明放在\ref{sub:fourier_continuous}中。
