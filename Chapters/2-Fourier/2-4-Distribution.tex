...
取$g=f$即证。
\end{proof}
相比\refsub{fourier_operation_properties}中的帕塞瓦尔定理，这个证明适用于$f$是正则分布而$g$是施瓦兹函数的情况，它的证明要简单得多。
...
分布的伸缩与函数的伸缩有一点不同，即多了一个$\dfrac{1}{|a|}$的因子，这意味着，作为形式上的函数定义的分布$T(x)$，其伸缩$\sigma_a T(x)$并不一定等于$T(ax)$。在\refsub{signal}中定义过
