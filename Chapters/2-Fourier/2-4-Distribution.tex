...
我们已经看到，分布（例如狄拉克$\delta$）难以用经典的“函数”来描述，这时我们可以考察它们与一系列\textbf{检验函数}(test function)$\varphi$的作用，具体来说：
\begin{definition}[分布]
    我们称无限阶可导的函数是\textbf{光滑}(smooth)的。记定义于$\mathbb{R}$上的复值光滑紧支函数集为$\mathcal{C} $。如果将检验函数集取为$\mathcal{C}$，我们将其对偶空间$\mathcal{D} $中的元素称为分布，并规定函数$f\in \mathcal{C} $所产生的分布为作用在$\mathcal{C}$上的泛函：
    $$T_f:\varphi\mapsto T_f(\varphi)=\int_{-\infty}^{\infty}f(x)\varphi(x)\,dx$$
    后文中，我们更多地使用\textbf{配对}(pairing)的记号：
    \begin{equation*}
        \left\langle T_f,\varphi\right\rangle=\int_{-\infty}^{\infty}f(x)\varphi(x)\,dx,\varphi\in\mathcal{C}
    \end{equation*}
    将$T_f$这样的分布称为\textbf{正则分布}，而将无法用紧支函数描述的分布称为\textbf{奇异分布}。注意，与内积不同，我们有意地不将$\varphi$取共轭，这使得配对具有双线性性。
\end{definition}
\begin{remark}
...
由于“狄拉克函数”严格来说并不能算作一种函数，$\delta$是一个奇异分布。容易验证这与我们一开始给出的$\delta$作为“函数”的性质是相符的：
\[\langle \delta,\varphi\rangle=\int_{-\infty}^{\infty}\delta(x)\varphi(x)\,dx=\int_{-\infty}^{\infty}\delta(x)\varphi(0)\,dx=\varphi(0)\]
\end{example}

下面来定义分布与函数的乘法和分布的导数。这一部分中，我们的原则是：\textbf{奇异分布与正则分布具有相同的性质}，换言之，只要能够对正则分布定义的算子，就能够对奇异分布做相同的定义。在讨论分布的傅里叶变换时，还将定义更多的算子，例如分布卷积、尺度变换等。
...
\end{align*}
因此对于任意的分布$T\in\mathcal{D} $，定义它与$g\in\mathcal{C} $的乘积$gT$由以下等式给出：
\begin{definition}[分布与函数的乘法]
    \begin{equation*}
    \left\langle gT,\varphi\right\rangle=\langle T,g\varphi\rangle
\end{equation*}
\end{definition}
$g\varphi$就是普通的函数乘法。现在就可以说，分布集$\mathcal{D} $构成函数环$\mathcal{C} $上的模(module)，并且可以验证$\delta$的采样性质：
\begin{align*}
...
\begin{example}[$\delta$的导数]\label{exmp:delta'}
\begin{align*}
...
\end{align*}
\end{example}
$\delta$的高阶导数可以依此类推，它们已经难以用类似$\delta(x)$的“函数”描述，但可以看到$\delta^{(n)}$作为一个泛函（分布）作用是取测试函数的$(-1)^n$倍的n阶导。
...
\end{enumerate}
可以看到，分布理论中极限的概念是通过测试函数来定义的，如果分布序列$\{T_m\}_{m=1}^{\infty}$作用在任何测试函数上都是趋于某个分布T作用于这个测试函数的值，就说序列$\{T_m\}$\textbf{弱收敛}(converge weakly)于T，并记为$T_m\to T$\cite{folland}。
\end{proof}
...
\end{equation}
然而，测试函数$\varphi\in\mathcal{C} $的傅里叶变换$\mathcal{F}\varphi$并不属于$\mathcal{C} $（关于这一点的说明，以及施瓦兹函数类的引出，见\refsub{fourier_close}），这说明测试函数集$\mathcal{C} $在傅里叶分析中的表现不够好，我们需要引入新的测试函数空间，以保证$\mathcal{F}\varphi$仍然是测试函数。这个测试函数空间正是\textbf{施瓦兹空间}(Schwartz space)$\mathcal{S} $\cite{brad_osgood_ee261}，它是$\mathbb{R}$上所有无限可微函数的集合，这些函数及其各阶导数都以比任何负幂更快的速度趋于0，即：
\begin{definition}[施瓦兹空间与施瓦兹函数]\label{def:schwartz}
    \[\mathcal{S} =\left\{\varphi\in C^{\infty}(\mathbb{R} )\left|\lim_{|x|\to\infty}|x|^m\varphi^{(n)}(x)=0,\forall m,n\in\mathbb{N} \right.\right\}\]
    施瓦兹空间中的函数称为\textbf{施瓦兹函数}(Schwartz function)，它们是非常光滑且衰减很快的函数，因此又称为\textbf{速降函数}(rapidly decreasing function)
\end{definition}
\begin{example}
高斯函数$\mathrm{e}^{-x^2}$及其各阶导数都属于施瓦兹空间。
\end{example}
\begin{definition}[施瓦兹分布]
    施瓦兹空间的对偶空间$\mathcal{T}:=\mathcal{S}^*$中的元素称为\textbf{施瓦兹分布}(Schwartz distribution)或\textbf{缓增分布}(tempered distribution)。称之为“缓增分布”，是因为一些能够用函数描述的施瓦兹分布在自变量趋于无穷时增长速度较缓，以至于它们与施瓦兹函数的乘积可积。
\end{definition}
我们仍定义
\begin{align}
    T(\varphi)                 & =\left\langle T,\varphi\right\rangle                                        \\
    \left\langle T_f,\varphi\right\rangle & =\int_{-\infty}^{\infty}f(x)\varphi(x)\,dx,\quad\varphi\in\mathcal{S}
\end{align}
易见$\mathcal{C} \subset \mathcal{S} ,\mathcal{T} \subset\mathcal{D}. $

有了施瓦兹函数类$\mathcal{S} $和缓增分布$\mathcal{T} $，我们就可以定义分布的傅里叶变换，还可以定义有关分布的一系列算子。前文中曾定义分布与函数的乘法和分布的导数，现在认为正则分布是由施瓦兹函数导出的，则显然能够推广到缓增分布，即\begin{align}
    \left\langle gT,\varphi\right\rangle=\left\langle T,g\varphi\right\rangle,\quad\left\langle T',\varphi\right\rangle=\left\langle T,\varphi'\right\rangle,\quad g\in\mathcal{S} ,T\in\mathcal{T}
\end{align}
...
\begin{align*}
    ...
    &=\int_{-\infty}^{\infty}\varphi(x)\,dx\int_{-\infty}^{\infty}f(y)\mathrm{e}^{-2\pi\mathrm{i}xy}\,dy\\
    & =\int_{-\infty}^{\infty}f(y)\,dy\int_{-\infty}^{\infty}\varphi(x)\mathrm{e}^{-2\pi \mathrm{i}xy}\,dx\\
    & =\int_{-\infty}^{\infty}f(y)\mathcal{F}\varphi(y)\,dy\\
    &=\left\langle T_f,\mathcal{F}\varphi\right\rangle
\end{align*}
因此我们定义\begin{definition}[分布的傅里叶变换]
    \begin{align*}
        \left\langle \mathcal{F} T,\varphi\right\rangle & =\left\langle T,\mathcal{F} \varphi\right\rangle,\quad T\in\mathcal{T} ,\varphi\in\mathcal{S}
    \end{align*}
\end{definition}
...
\begin{definition}[分布的傅里叶反变换]
    \begin{align*}
        \left\langle \mathcal{F} ^{-1}T,\varphi\right\rangle & =\left\langle T,\mathcal{F} ^{-1}\varphi\right\rangle,\quad T\in\mathcal{T} ,\varphi\in\mathcal{S}
    \end{align*}
\end{definition}

只要承认函数的傅里叶反演公式，分布的傅里叶反演公式就自然成立：\begin{theorem}[分布的傅里叶反演公式]
    \[\mathcal{F} \mathcal{F} ^{-1}T=\mathcal{F} ^{-1}\mathcal{F} T=T,\quad\forall T\in\mathcal{T}\]
\end{theorem}
\begin{proof}
    \begin{align*}
...
\begin{theorem}[分布的傅里叶变换的线性]
    \[\mathcal{F} (aT+bS)=a\mathcal{F} T+b\mathcal{F} S,\quad a,b\in\mathbb{C},S,T\in\mathcal{T}\]
\end{theorem}
\begin{proof}
...
根据阿贝尔-狄利克雷判别法（A-D判别法），$\langle \mathrm{e}^{-\mathrm{i} a x},\varphi\rangle$作为一个反常积分是有意义的。可以看出这与函数的傅里叶变换的平移定理很相似，我们将在后面给出分布的平移、伸缩，并由此得到一些分布的傅里叶变换的性质。
\end{example}
...
\lr{
    ...
    &=\mathcal{F} \mathcal{F} \varphi(0)\\
    &=2\pi\varphi(0)\\
    &=2\pi\left\langle \delta,\varphi\right\rangle
}
因此\lr{
...
因此我们定义\begin{definition}[分布的反转]
    \begin{equation*}
    \left\langle T^-,\varphi\right\rangle=\left\langle T,\varphi^-\right\rangle,\quad T\in\mathcal{T} ,\varphi\in\mathcal{S}
\end{equation*}
\end{definition}

有了反转就可以定义分布的奇偶性：
\begin{definition}[分布的奇偶性]
    如果$T^-=T$，则$T$是\textbf{偶分布}；如果$T^-=-T$，则$T$是\textbf{奇分布}。
\end{definition}
...
利用分布的傅里叶变换的定义及其对偶性，可以立即得到：
\begin{theorem}[帕塞瓦尔恒等式/普朗歇尔定理]\label{thm:distribution_plancherel}
    设$f\in\mathcal{T} ,g\in\mathcal{S} $，则
    \lr{
        \int_{-\infty}^{\infty}f(t)\overline{g(t)}\,dt=\int_{-\infty}^{\infty}\mathcal{F} f(\xi)\overline{\mathcal{F}g(\xi)}\,d\xi
    }{
        \int_{-\infty}^{\infty}f(t)\overline{g(t)}\,dt=\frac{1}{2\pi}\int_{-\infty}^{\infty}\mathcal{F} f(\omega)\overline{\mathcal{F}g(\omega)}\,d\omega
    }
\end{theorem}
\begin{proof}
\begin{lralign}
...
\end{example}
\begin{example}[复指数函数$\mathrm{e}^{\mathrm{i}at}$的傅里叶变换]
作为函数当然不能求$\mathrm{e}^{\mathrm{i}at}$的傅里叶变换，我们甚至无法确定它在0处的傅里叶变换：$\int_{-\infty}^{\infty}\mathrm{e}^{\mathrm{i}at}\,dt$不存在。
但它可以视为一个缓增分布，前文中已经提到$\langle \mathrm{e}^{\mathrm{i}at},\varphi\rangle$是收敛
的。下面应用对偶性求它的傅里叶变换：
\lr{
\mathcal{F} [\e^{2\pi \mathrm{i}at}]=\mathcal{F}^{-1}[\e^{2\pi \mathrm{i}at}]^-=\delta_{-a}^-=\delta_{a}
}{
\mathcal{F} [\e^{\mathrm{i}at}]=2\pi\mathcal{F}^{-1}[\e^{\mathrm{i}at}]^- =2\pi\delta_{-a}^- =2\pi\delta_{a}
}
（请读者自行验证$\delta_{-a}^-= \delta_{a}$）。
\end{example}

\begin{example}[正弦、余弦函数的傅里叶变换]
根据欧拉公式$\mathrm{e}^{\im x}=\cos(x)+\mathrm{i}\sin(x)$，
有\[\cos(x)=\frac{\mathrm{e}^{\im x}+\mathrm{e}^{-\im x}}{2},\quad\sin(x)=\frac{\mathrm{e}^{\im x}-\mathrm{e}^{-\im x}}{2\mathrm{i}}\]因此
\lr{
...
因此我们定义\begin{definition}[分布的共轭]
    \begin{equation*}
    \left\langle \overline{T},\varphi\right\rangle=\overline{\left\langle T,\overline{\varphi}\right\rangle},\quad T\in\mathcal{T} ,\varphi\in\mathcal{S}
\end{equation*}
\end{definition}
...
\begin{definition}[分布的实部、虚部]
    设$T\in\mathcal{T} $，则
    \[\mathrm{Re}(T)=\frac{T+\overline{T}}{2}\]
    称为T的\textbf{实部}，
    \[\mathrm{Im}(T)=\frac{T-\overline{T}}{2\im}\]
    称为T的\textbf{虚部}。
\end{definition}
...
\end{align*}
因此我们定义\begin{definition}[分布的平移]\label{def:distribution_delay}
    \begin{equation*}
    \left\langle \tau_b T,\varphi\right\rangle=\left\langle T,\tau_{-b}\varphi\right\rangle,\quad T\in\mathcal{T} ,\varphi\in\mathcal{S}
\end{equation*}
\end{definition}
...
因此我们定义\begin{definition}[分布的伸缩]
    \begin{equation*}
    \left\langle\sigma_a T,\varphi\right\rangle=\left\langle T,\frac{1}{|a|}\sigma_{1/a}\varphi\right\rangle,\quad T\in\mathcal{T} ,\varphi\in\mathcal{S}
\end{equation*}
\end{definition}

分布的伸缩与函数的伸缩有一点不同，即多了一个$\dfrac{1}{|a|}$的因子，这意味着，作为形式上的函数定义的分布$T(x)$，其伸缩$\sigma_a T(x)$并不一定等于$T(ax)$。我们在\refsub{signal}中声明：
$$\delta(ax)=\dfrac{1}{|a|}\delta(x)$$
实际上它是将$\delta$作为一个分布，套用分布的伸缩的定义：
...
\begin{figure}[ht]
    ...
    \begin{tikzpicture}[scale=1, every node/.style={font=\small}]
        ...
        \node[align=center] (Llabel) at (Lstart) {\textbf{$L^1(\mathbb{R})$函数/正则分布}\\\small (函数的伸缩有定义)};
        \node[align=center] (Rlabel) at (Rstart) {\textbf{分布}\\\small{(分布的伸缩有定义)}};
...
上图中，像$\delta$这样的从形式上定义的函数，应该归类于奇异分布。在\refsub{ordinary_signal}中给出$\delta(ax)$的定义时，我们实际上考察的是它作为分布的伸缩性质，这也是为什么$\delta$的两种伸缩定义看起来是一致的。

有了分布的平移和伸缩的定义，就可以建立分布的傅里叶变换的平移和尺度变换定理：
\begin{theorem}[分布的傅里叶变换的平移]
\begin{lralign}
    &\mathcal{F}(\tau_b T)=\mathrm{e}^{-2\pi\mathrm{i}bx}\mathcal{F}T\\
    &\mathcal{F}(\e^{2\pi\im b t}T)=\tau_b\mathcal{F}T
    \switch
    &\mathcal{F}(\tau_b T)=\mathrm{e}^{-\mathrm{i}bx}\mathcal{F}T\\
    &\mathcal{F}(\e^{\im b t}T)=\tau_b\mathcal{F}T
\end{lralign}
\end{theorem}
\begin{proof}
    \ding{172}时移性质
    \begin{lralign}
        \left\langle\mathcal{F}(\tau_b T),\varphi\right\rangle &=\left\langle \tau_b T,\mathcal{F}\varphi\right\rangle\\
        &=\left\langle T,\tau_{-b}\mathcal{F}\varphi\right\rangle\\
        &=\left\langle T,\mathcal{F}[\mathrm{e}^{-2\pi \mathrm{i}bx}\varphi]\right\rangle\\
        &=\left\langle \mathrm{e}^{-2\pi\mathrm{i}bx}\mathcal{F}T,\varphi\right\rangle
    \switch
        \left\langle\mathcal{F}(\tau_b T),\varphi\right\rangle &=\left\langle \tau_b T,\mathcal{F}\varphi\right\rangle\\
        &=\left\langle T,\tau_{-b}\mathcal{F}\varphi\right\rangle\\
        &=\left\langle T,\mathcal{F}[\mathrm{e}^{-\mathrm{i}bx}\varphi]\right\rangle\\
        &=\left\langle \mathrm{e}^{-\mathrm{i}bx}\mathcal{F}T,\varphi\right\rangle
    \end{lralign}
即
    \begin{lralign}
        \mathcal{F}(\tau_b T)=\mathrm{e}^{-2\pi\mathrm{i}bx}\mathcal{F}T
    ...
    \ding{173}频移性质
    \begin{lralign}
        \langle \mathcal{F}(\e^{2\pi\im b t}T),\varphi\rangle&=\langle T,\e^{2\pi\im b t}\mathcal{F}\varphi\rangle\\
        &=\langle T,\mathcal{F}(\tau_{-b}\varphi)\rangle\\
        &=\langle \tau_b\mathcal{F}T,\varphi\rangle
    \switch
        \langle \mathcal{F}(\e^{\im b t}T),\varphi\rangle&=\langle T,\e^{\im b t}\mathcal{F}\varphi\rangle\\
        &=\langle T,\mathcal{F}(\tau_{-b}\varphi)\rangle\\
        &=\langle \tau_b\mathcal{F}T,\varphi\rangle
    \end{lralign}
    即\begin{lralign}
...
\begin{theorem}[分布的傅里叶变换的尺度变换]
    $$\mathcal{F}(\sigma_a T)=\frac{1}{|a|}\sigma_{1/a}\mathcal{F}T$$
\end{theorem}
\begin{proof}
    \begin{align*}
    \left\langle\mathcal{F}(\sigma_a T),\varphi\right\rangle =\left\langle \sigma_a T,\mathcal{F}\varphi\right\rangle=\left\langle T,\frac{1}{|a|}\sigma_{\frac{1}{a}}\mathcal{F}\varphi\right\rangle=\left\langle T,\mathcal{F}[\sigma_a \varphi]\right\rangle=\left\langle \mathcal{F}T,\sigma_a\varphi\right\rangle=\left\langle \frac{1}{|a|}\sigma_{1/a}\mathcal{F}T,\varphi\right\rangle
\end{align*}
即\[\mathcal{F}(\sigma_a T)=\frac{1}{|a|}\sigma_{1/a}\mathcal{F}T\]
\end{proof}

\subsubsection{分布的傅里叶变换的微分性质}
...
\switch
    ...
    &=\left\langle -\mathcal{F} (\mathrm{i}tT),\varphi\right\rangle
\end{lralign}
因此
\begin{lralign}
    (\mathcal{F} T)'=-\mathcal{F} (2\pi \mathrm{i} tT)
\switch
    (\mathcal{F} T)'=-\mathcal{F} (\mathrm{i}tT)
\end{lralign}
也即
\begin{lralign}
...
\end{align*}
即\[(f*g)*T=f*(g*T),\quad f,g\in\mathcal{S} ,T\in\mathcal{T}\]
...
\end{align*}
即\[(af+bg)*T=af*T+bg*T,\quad a,b\in\mathbb{C},f,g\in\mathcal{S} ,T\in\mathcal{T} \]
...
\end{align*}
即\[f*(aS+bT)=af*S+bf*T,\quad a,b\in\mathbb{C},f\in\mathcal{S} ,S,T\in\mathcal{T}\]
...
综上我们得到：\begin{theorem}[分布与函数的卷积定理]
    \label{thm:distribution_func_conv}
    \quad
    \lr{
    &\mathcal{F} (g*T)=\mathcal{F} g\mathcal{F} T \\
    &\mathcal{F} (gT)=\mathcal{F} g*\mathcal{F} T
...
这与函数之间卷积的情况是完全一样的。对于$T\in\mathcal{T}$，保证$g*T$存在的一种典型条件是$g\in\mathcal{S}$.
...
\begin{definition}[分布与分布的卷积]\label{def:conv_distr_distr}
    \begin{equation*}
    \left\langle S*T,\varphi\right\rangle=\left\langle S(y),\left\langle T(x),\varphi(x+y)\right\rangle\right\rangle,S,T\in\mathcal{T} ,\quad\varphi\in\mathcal{S} ,\left\langle T(x),\varphi(x+y)\right\rangle\in\mathcal{S}
\end{equation*}
\end{definition}
...
\begin{example}[结合律的失效]\label{exmp:associative_loss}
\begin{align*}
...
\subsection{再看正则分布}\label{sub:normal_distribution}

在\refsub{fourier_of_signal}中，我们已经从形式上得到了$\mathrm{sgn},u,1/t$等$L^1(\mathbb{R})$以外的函数的傅里叶变换，本小节借助分布理论验证这些结果。

\subsubsection{采样函数}

在\refsub{int_sin}中已经说明：采样函数$\sinc\notin L^1(\mathbb{R})$，因此它的傅里叶变换是在分布的意义下求得的，利用分布的傅里叶变换的对偶性可以说明这一点。

\subsubsection{符号函数和单位阶跃函数}

利用分布的傅里叶变换的微分性质，有：
\begin{lralign}
    \mathcal{F}(\sgn')&=2\pi\im\xi\mathcal{F} \sgn=2
    \switch
    \mathcal{F}(\sgn')&=\im\omega\mathcal{F} \sgn=2
\end{lralign}

这是一个分布方程，它确定了$\mathcal{F}\sgn$在原点以外的值，但不应当认为
\begin{lralign}
    \mathcal{F} \sgn&=\frac{1}{\pi\im\xi}
    \switch
    \mathcal{F} \sgn&=\frac{2}{\im\omega}
\end{lralign}
因为反比例函数不能完整地描述分布$\mathcal{F}\sgn$的行为，它在原点附近不可积，通常无法与施瓦兹函数配对。为了处理这个问题，我们认为反比例函数作为一个正则分布的作用是做主值意义下的积分，由于施瓦兹函数光滑，这个主值积分总是收敛的。为了体现这一点，我们将它写成
\begin{lralign}
    \pv\frac{1}{\pi\im\xi}
    \switch
    \pv\frac{2}{\im\omega}
...
这样，在原点以外，它仍然满足分布方程：
\begin{lralign}
    \pi\im\xi\pv\frac{1}{\pi\im\xi}&=1
    \switch
    \im\omega\pv\frac{2}{\im\omega}&=2
\end{lralign}

为了描述反比例函数在原点处趋于无穷的行为，需要借助$\delta$分布。设
\begin{lralign}
    \mathcal{F} \sgn&=\pv\frac{1}{\pi\im\xi}+c\delta
    \switch
    \mathcal{F} \sgn&=\pv\frac{2}{\im\omega}+c\delta
\end{lralign}

为了确定$c$，我们尝试引入另一个分布方程。由于
$$\sgn(t)+\sgn(-t)=0$$
利用傅里叶变换的反转性质，得到：
\begin{lralign}
    \mathcal{F} \sgn+\mathcal{F} \sgn(-t)&=0\\
    \pv\frac{1}{\pi\im\xi}+c\delta-\pv\frac{1}{\pi\im\xi}+c\delta&=0\\
    2c\delta&=0
    \switch
    \mathcal{F} \sgn+\mathcal{F} \sgn(-t)&=0\\
    \pv\frac{2}{\im\omega}+c\delta-\pv\frac{2}{\im\omega}+c\delta&=0\\
    2c\delta&=0
\end{lralign}
可见$c=0$，因此：
\begin{lralign}
    \mathcal{F} \sgn&=\pv\frac{1}{\pi\im\xi}
    \switch
    \mathcal{F} \sgn&=\pv\frac{2}{\im\omega}
\end{lralign}
...
可得
\begin{lralign}
    \mathcal{F} u&=\frac{1}{2}\left(\pv\frac{1}{\pi\im\xi}+\delta\right)
    \switch
    \mathcal{F} u&=\pv\frac{1}{\im\omega}+\pi\delta
\end{lralign}

这与我们在\cref{cor:sgn}中得到的结果大致相同\footnote{在考试时请不要把$\pv$写在卷子上。}。

\subsubsection{反比例函数}

和前面的$\mathcal{F}\sgn$一样，$1/t$在原点附近不可积，因而应理解为主值分布$\pv\dfrac{1}{t}$.利用分布的傅里叶变换的对偶性，对任意$\varphi\in\mathcal{S}$有：
\begin{lralign}
    \left\langle\mathcal{F}\left(\pv\frac{1}{t}\right),\varphi\right\rangle
    &=\left\langle\pv\frac{1}{t},\mathcal{F}\varphi\right\rangle\\
    &=\pi\im\left\langle\mathcal{F}\sgn,\mathcal{F}\varphi\right\rangle\\
    &=\pi\im\left\langle\sgn,\mathcal{F}\mathcal{F} \varphi\right\rangle\\
    &=-\pi\im\left\langle\sgn,\varphi\right\rangle
    \switch
    \left\langle\mathcal{F}\left(\pv\frac{1}{t}\right),\varphi\right\rangle
    &=\left\langle\pv\frac{1}{t},\mathcal{F}\varphi\right\rangle\\
    &=\frac{\im}{2}\left\langle\mathcal{F}\sgn,\mathcal{F}\varphi\right\rangle\\
    &=\frac{\im}{2}\left\langle\sgn,\mathcal{F}\mathcal{F} \varphi\right\rangle\\
    &=-\pi\im\left\langle\sgn,\varphi\right\rangle
\end{lralign}
因此
\begin{lralign}
    \mathcal{F}\left(\pv\frac{1}{t}\right)&=-\pi\im\sgn(\xi)
    \switch
    \mathcal{F}\left(\pv\frac{1}{t}\right)&=-\pi\im\sgn(\omega)
\end{lralign}
这便严格给出了\cref{cor:1/t}中反比例函数的傅里叶变换；其中$1/t$始终应理解为主值分布。

\subsubsection{多项式}

利用分布的傅里叶变换的微分性质，可依次求出各阶单项式的傅里叶变换：
\begin{lralign}
    \mathcal{F}(t^n)&=\left(\frac{\im}{2\pi}\right)^n\delta^{(n)},\quad n=0,1,2,\ldots
    \switch
    \mathcal{F}(t^n)&=2\pi\im^n\delta^{(n)},\quad n=0,1,2,\ldots
\end{lralign}
事实上，若结论对$n$成立，则
\begin{lralign}
    \mathcal{F}(t^{n+1})&=\frac{\im}{2\pi}\left[\mathcal{F}(t^n)\right]'=\left(\frac{\im}{2\pi}\right)^{n+1}\delta^{(n+1)}
    \switch
    \mathcal{F}(t^{n+1})&=\im\left[\mathcal{F}(t^n)\right]'=2\pi\im^{n+1}\delta^{(n+1)}
\end{lralign}
因此，若$P(t)=\sum_{k=0}^{n}a_k t^k$为多项式，则
\begin{lralign}
    \mathcal{F}P&=\sum_{k=0}^{n}a_k\left(\frac{\im}{2\pi}\right)^k\delta^{(k)}
    \switch
    \mathcal{F}P&=2\pi\sum_{k=0}^{n}a_k\im^k\delta^{(k)}
\end{lralign}
可见，多项式的频谱集中在零频率处；次数越高，所对应的$\delta$分布的导数阶数也越高。

\subsection{分布的傅里叶变换的连续性}\label{sub:continuity_of_distribution}

\begin{theorem}[分布的傅里叶变换的连续性]
    \label{thm:distribution_fourier_continuous}
    设$\{T_n\}_{n=1}^\infty\subset\mathcal{T} $，$T\in\mathcal{T} $，如果$T_n\to T$，则$\mathcal{F} T_n\to\mathcal{F} T$.
\end{theorem}
\begin{proof}
    设$\varphi\in\mathcal{S} $，则
    \begin{align*}
        \lim_{n\to\infty}\left\langle\mathcal{F} T_n,\varphi\right\rangle & =\lim_{n\to\infty}\left\langle T_n,\mathcal{F} \varphi\right\rangle=\langle T,\mathcal{F}\varphi\rangle=\langle \mathcal{F}T,\varphi\rangle
    \end{align*}
\end{proof}

分布的傅里叶变换的连续性是在弱收敛的意义下定义的，也就是说，作为一个分布，我们总是能够交换极限或求和与傅里叶变换的顺序（我们将会多次使用到这一结论），但是对于正则分布列$f_n\to f$，尽管$T_{f_n}\to T_f$，也不能认为$\mathcal{F}f_n$作为一个函数列收敛于$\mathcal{F}f$。在\refsec{Sampling_Interpolation}中我们将看到，通过分布的傅里叶变换的连续性，$\shah$分布能表示成傅里叶级数的形式，然而这个展开式是几乎处处不收敛的函数项级数。

事实上，施瓦兹函数的傅里叶变换也是具有连续性的。然而，证明它要困难得多，我们将证明放在\refsub{fourier_continuous}中。
