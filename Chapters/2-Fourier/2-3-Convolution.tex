...
\begin{proof}
\begin{align*}
    (f*g)*h(x) & =\int_{-\infty}^{\infty}(f*g)(x-y)h(y)\,dy\\
    &=\int_{-\infty}^{\infty}h(y)\,dy\int_{-\infty}^{\infty}f(z)g(x-y-z)\,dz \\
    & =\int_{-\infty}^{\infty}\int_{-\infty}^{\infty}f(z)g(x-y-z)h(y)\,dy\,dz\\
    &=\int_{-\infty}^{\infty}f(z)(g*h)(x-z)\,dz
\end{align*}
\end{proof}
...
卷积是否具有“幺元”呢？我们希望找到一个函数，使得它与任一函数$f$卷积，总得到$f$本身。事实上，只有$\delta$可以起到这个作用（尽管它应该作为一个分布来理解，见\refsub{general_distributions}和\refsub{approach}）。
...
初次接触卷积时，往往会对它的定义感到疑惑，因为在数学分析的课程中我们很少见到这种“翻转、平移、相乘、积分”的结构。需要指出，卷积并不只有“时域相乘，频域卷积；时域卷积，频域相乘”的物理意义，例如概率论中两独立的连续型随机变量X,Y之和作为一种新的随机变量Z，其概率密度函数$f_Z(z)$正是两个独立的随机变量的概率密度函数的卷积$f_X*f_Y(z)$.类似于用“求曲线下方的面积”或“已知速度求位移”引入积分，尽管我们用一种较为自然的方式引入了卷积，但\textbf{不应该认为它只有单一的意义}。不过，还是可以建立一些卷积的性质来辅助我们理解卷积。

\subsubsection{卷积是一种起“平均化”作用的运算}
...
\subsubsection{卷积函数的支集}

%我们首先引入\textbf{支集}(support)的概念，读者只需理解其直观，真正理解它需要一些拓扑学的基础。
\begin{definition}[支集]
    设$f:\mathbb{R}\to \mathbb{R}$，f的\textbf{支集}为
$\mathrm{supp}(f)=\overline{\left\{x\in\mathbb{R}\left|f(x)\neq 0\right.\right\}}$
\end{definition}
...
\begin{proposition}
    \label{prop:conv_supp}
    卷积能够将两个函数的支集“相加”，即：\begin{align*}
    \mathrm{supp}(f)\in [a,b],\mathrm{supp}(g)\in [c,d]\implies&\mathrm{supp}(f*g)\in [a+c,b+d]
    \end{align*}
\end{proposition}
...
在建立卷积在信号时延或伸缩下的性质前，我们先来明确所使用的符号。我们将用$\tau$表示时延$\tau_b f(t)=f(t-b)$，用$\sigma$表示伸缩$\sigma_a f(t)=f(at)$，并避免使用$\tau,\sigma$作为变量的符号。这样的做法在研究分布的性质时是必要的，因为严格来讲不能给出分布的“自变量”，但例如$\delta$函数这样的分布又具有明显的尺度变换的性质，见\refsub{operation_of_distribution}.

\begin{proposition}[信号时延、伸缩时的卷积]
    \label{prop:conv_signal_shift}
    \begin{align*}
...
\begin{equation*}
    \rho(f,g)=\frac{(f,g)}{\|f\|\cdot\|g\|}
\end{equation*}
\end{definition}
其中$(f,g)=\int_{-\infty}^{\infty}f(x)\overline{g(x)}dx$是$f$和
$g$的内积，$\|f\|=\sqrt{(f,f)}$是$f$的范数。根据柯西-施瓦茨不等式，
...
\begin{equation*}
    R_{fg}(x)=\int_{-\infty}^{\infty}f(x+y)\overline{g(y)}\,dy
\end{equation*}
同样可以定义f的\textbf{自相关}(auto-correlation)$R_f(x)$
\end{definition}

互相关具有以下性质：
\begin{proposition}
    \label{prop:cross_correlation}
    设$f,g\in L^2(\mathbb{R})$，则
\begin{enumerate}
    \item $R_{fg}=f* \overline{g^-}=\overline{R_{gf}^-}$，如果$f,g$都是实信号，则$R_{fg} =R_{gf}^- =f^- *g=(f*g^-)^-$.该性质表明相关与卷积有着密切的联系，并且互相关函数具有共轭对称性。
    \item (维纳-辛钦定理)设$f,g\in L^1(\mathbb{R})$，则$\mathcal{F} R_{fg} =\mathcal{F} f\overline{\mathcal{F} g}$
    \item $R_{\tau_b f,g}=\tau_{b}R_{fg}=R_{f,\tau_{-b}g}$
    \item $R_{fg}\leq\|f\|_2\|g\|_2$，特别地，$R_f(x)\leq R_f(0)=\|f\|_2^2$
\end{enumerate}
\end{proposition}
\begin{remark}
    一般来说，\textbf{维纳-辛钦定理}(Wiener-Khinchin Theorem)是指第二条性质的特殊情况，即令$f=g$，得到
    $$\mathcal{F}(R_f)=|\mathcal{F} f|^2$$
    但是，用这个名称来指代更一般的定理$\mathcal{F}R_{fg}=\mathcal{F}f\overline{\mathcal{F}g}$是方便的，因此本书总是这样做。
\end{remark}
...
\begin{align*}
    R_{fg}(x)&=\int_{-\infty}^{\infty}f(x+y)\overline{g(y)}\,dy\\
    &=\int_{-\infty}^{\infty}f(x-y)\overline{g(-y)}\,dy&(y\to -y)\\
    &=f*\overline{g^-}
\end{align*}
\begin{align*}
    R_{fg}(x)& =\int_{-\infty}^{\infty}f(x+y)\overline{g(y)}\,dy\\
    &=\overline{\int_{-\infty}^{\infty}\overline{f(y)}g(y-x)\,dy}&(y\to y-x)\\
    &=\overline{\overline{f}*g(x)}\\
    &=\overline{\int_{-\infty}^{\infty}g(y-x)\overline{f(y)}\,dy}\\
    &=\overline{R_{gf}}(-x)
\end{align*}

由于傅里叶变换时不涉及频率、角频率的问题，我们仅给出一种符号下的证明：

2.
\begin{align*}
    \mathcal{F} R_{fg}(\xi) & =\int_{-\infty}^{\infty}R_{fg}(x)\mathrm{e}^{-2\pi \mathrm{i}\xi x}\,dx\\
    & =\int_{-\infty}^{\infty}\mathrm{e}^{-2\pi \mathrm{i}\xi x}\,dx\int_{-\infty}^{\infty}f(x+y)\overline{g(y)}\,dy\\
    & =\int_{-\infty}^{\infty}\overline{g(y)}\,dy\int_{-\infty}^{\infty}f(x+y)\mathrm{e}^{-2\pi \mathrm{i}\xi x}\,dx\\
    & =\int_{-\infty}^{\infty}\overline{g(y)}\mathrm{e}^{2\pi \mathrm{i}\xi y}\,dy\int_{-\infty}^{\infty}f(u)\mathrm{e}^{-2\pi \mathrm{i}\xi u}\,du & (u=x+y) \\
    &=\overline{\int_{-\infty}^{\infty}g(y)\mathrm{e}^{-2\pi \mathrm{i}\xi y}\,dy}\int_{-\infty}^{\infty}f(u)\mathrm{e}^{-2\pi \mathrm{i}\xi u}\,du\\
    & =\mathcal{F} f(\xi)\overline{\mathcal{F} g(\xi)}
\end{align*}

3.
\begin{align*}
    R_{\tau_b f,g}(x)&=\int_{-\infty}^{\infty}\tau_b f(x+y)\overline{g(y)}\,dy\\
    & =\int_{-\infty}^{\infty}f(x+y-b)\overline{g(y)}\,dy\\
    & =R_{fg}(x-b)=\tau_{b} R_{fg}(x)\\
    &=\int_{-\infty}^{\infty}f(x+u)\overline{g(u+b)}\,du&(u=y-b)\\
    &=R_{f,\tau_{-b}g}
\end{align*}

4.\begin{align*}
    R_{fg}(x)& =\int_{-\infty}^{\infty}f(x+y)\overline{g(y)}\,dy\\
    & \leq \sqrt{\int_{-\infty}^{\infty}|f(x+y)|^2\,dy\int_{-\infty}^{\infty}\left|\overline{g(y)}\right|^2\,dy  }\\
    &=\sqrt{\|f\|_2^2\|g\|_2^2}\\
    &=\|f\|_2\cdot\|g\|_2  
\end{align*}
\end{proof}

一些文献（例如\cite{brad_osgood_ee261}）将互相关函数定义为
$$R_{fg}(x)=\int_{-\infty}^{\infty}f(y)\overline{g(x+y)}\,dy$$
这与上面讨论的互相关没有太大的区别，但维纳-辛钦定理的形式会更加复杂。

对于功率信号，我们需要对以上互相关的定义略作修改，以避免趋于无穷的情况发生。
\begin{definition}[功率信号的互相关]
    对于功率信号$f,g$，定义互相关函数为
\begin{equation*}
    R_{fg}=R_{fg}(x)=\lim_{T\to\infty}\frac{1}{2T}\int_{-T}^{T}f(x+y)\overline{g(y)}\,dy
\end{equation*}
\end{definition}
我们将在\refsub{power_corelation}中讨论它。
...
\begin{example}[雷达测距]
    ...
    \[R_{fn}(t)=C\]
    C为常数（对于随机信号来说，这是可以做到的），因此可以考虑求发射信号与接收信号的互相关函数：
    \begin{align*}
        R_{f,f_r}(t)&=R_{f,\tau_{2T}f}(t)+R_{fn}(t)\\
        &=\alpha\tau_{2T}R_f(t)+\alpha C
    \end{align*}
    根据前面得到的性质4，$R_f(t)$在0处取最大值，因此$R_{f,f_r}(t)$在$2T$处取最大值，于是我们只需要观察这个互相关函数的最大值点，就可以得到T。
\end{example}
