...
\begin{definition}[互相关]
    ...
    \end{equation*}
    当$f=g$时，称之为$f$的\textbf{自相关}(auto-correlation)并记为$R_f(x)$.
\end{definition}
...
\begin{proposition}
    ...
    \begin{enumerate}
        \item $R_{fg}=f* \overline{g^-}=\overline{R_{gf}^-}$，特别地，如果$f,g$都是实信号，则
              $$R_{fg} =R_{gf}^- =f *(g^-)=((f^-)*g)^-$$
              该性质表明相关与卷积有着密切的联系，并且互相关函数具有共轭对称性。
...
从相关的第一条性质可见，相关也可以看作卷积，利用卷积定理可得：

\begin{theorem}[维纳-辛钦定理]\label{thm:winner-khinchin}
    设$f,g\in L^1(\mathbb{R})\cap L^2(\mathbb{R})$，则$\mathcal{F} R_{fg} =\mathcal{F} f\overline{\mathcal{F} g}$
\end{theorem}
\begin{remark}
    一般来说，\textbf{维纳-辛钦定理}(Wiener-Khinchin Theorem)是令$f=g$得到的
    $$\mathcal{F}(R_f)=|\mathcal{F} f|^2$$
    但是，用这个名称来指代更一般的定理$\mathcal{F}R_{fg}=\mathcal{F}f\overline{\mathcal{F}g}$是方便的，因此本书总是这样做。
\end{remark}
\begin{proof}
    $R_{fg}(x)=f*\overline{g^-}$，以频率形式为例，计算$\overline{g^-}$的傅立叶变换：
    \begin{align*}
        \mathcal{F}\left(\overline{g^-}\right)= & \int_{-\infty}^\infty \overline{g^-(t)}\e^{2\pi\im\xi t}\,dt \\
        =                                       & \overline{\int_{-\infty}^\infty g(-t)\e^{-2\pi\im\xi t}\,dt} \\
        =                                       & \overline{\int_{-\infty}^\infty g(t)\e^{2\pi\im\xi t}\,dt}   \\
        =                                       & \overline{\mathcal{F}g}
    \end{align*}
    由卷积定理可得：
    $$\mathcal{F}R_{fg}=\mathcal{F}(f*g^-)=\mathcal{F}(f)\overline{\mathcal{F}g}$$
\end{proof}
...
\begin{definition}[功率信号的互相关]
    ...
    \end{equation*}
\end{definition}
容易根据柯西-施瓦兹不等式证明其有界性，然而有界不代表极限存在。我们将在\refsub{power_corelation}中讨论它。
