...
\begin{equation*}
    R_{fg}(x)=(f\star g)(x)=\int_{-\infty}^{\infty}f(y)\overline{g(x+y)}\,dy
\end{equation*}
同样可以定义f的自相关$R_f(x)=(f\star f)(x)$
\end{definition}

我们给出了两种相关的符号，其中$R_{fg}$在信号与系统教材中被广泛使用，但$(f\star g)$更便于讨论相关的性质。
...
\begin{enumerate}
    ...
    \item (维纳-辛钦定理)设$f,g\in L^1(\mathbb{R})$，则$\mathcal{F} (f\star g) =\mathcal{F} f\overline{\mathcal{F} g}$
...
\end{align*}
这里不涉及频率、角频率的问题，读者可以自行验证。

2.
\begin{align*}
    \mathcal{F} (f\star g)(\xi) & =\int_{-\infty}^{\infty}(f\star g)(x)\mathrm{e}^{-2\pi \mathrm{i}\xi x}\,dx\\
...
3.
\begin{align*}
    \left(f\star (\tau_b g)\right)(x)&=\int_{-\infty}^{\infty}f(y)\overline{(\tau_b g)(x+y)}\,dy\\
...
\begin{definition}[功率信号的互相关]
    对于功率信号$f,g$，定义互相关函数为
\begin{equation*}
    R_{fg}=(f\star g)(x)=\lim_{T\to\infty}\frac{1}{2T}\int_{-T}^{T}f(y)\overline{g(x+y)}\,dy
\end{equation*}
\end{definition}
