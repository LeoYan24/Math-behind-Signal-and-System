...
$$\Omega=2\pi\frac{k}{N}$$
这表明，离散信号$E_{\Omega}[n]$具有周期性的必要条件是$\Omega/\pi$是有理数；另一方面，当$\Omega/\pi$是有理数时，容易验证$E_{\Omega}[n]$是周期离散信号。综上我们得到：
...
另外，对于$\Omega=\pi\frac{p}{q},p,q\in\mathbb{Z}$，且$p/q$为既约分数，容易验证$E_{\Omega}[n]$的最小正周期为$2q$.
...
\begin{proof}
    ...
    \begin{align*}
        \mathrm{i}\dfrac{dX(\mathrm{e}^{\mathrm{i}\Omega})}{d\Omega}&=\mathrm{i}\sum_{n=-\infty}^{\infty}x[n]\frac{d}{d\Omega}\mathrm{e}^{-\mathrm{i}\Omega n}\\
        &=\sum_{n=-\infty}^{\infty}nx[n]\mathrm{e}^{-\mathrm{i}\Omega n}
    \end{align*}
    它与z域微分性质的形式不同，是因为涉及到复合函数的求导。
\end{proof}
...
则右式是周期函数的卷积（见\refsub{conv_of_period_func}）：
