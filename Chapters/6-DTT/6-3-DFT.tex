...
本小节的主要目标——\textbf{离散傅里叶变换} (Discrete Fourier Transform, DFT)就是求解离散傅里叶级数的系数的这个变换，只是习惯上认为离散傅里叶变换是针对于\textbf{有限长离散信号}而言的\cite{dsp}，但很明显，我们\textbf{没有必要区分周期信号与有限长离散信号}，它们之间的互相转化是非常简单的。
...
\begin{definition}[离散信号的反转]
    将离散信号$x[n]$的反转信号$x[-n]$记为$x^-[n]$。特别地，将离散周期信号的反转信号写成向量形式，有：
    \begin{align*}
...
\begin{align*}
    x[n]=f\left(\frac{nT}{N}\right)&=\sum_{k=-\infty}^{\infty}c_k \mathrm{e}^{\mathrm{i}\frac{2\pi}{T}kn\frac{T}{N}}\\
...
\end{align*}
左侧括号内是狄拉克梳状分布的傅里叶级数形式，通过（角频率形式的）傅里叶变换可以推导出它的等价形式。由分布的傅里叶变换的连续性（\cref{thm:distribution_fourier_continuous}），有：
\begin{align*}
...
假设$f(t)$是在时域、频域上同时紧支的连续信号（在\refsub{time_freq_limit}中我们会看到，这样的信号是不存在的，但近似满足这种关系的信号却很多），对$f(t)$进行理想采样时，将得到一个有限长离散信号$x[n]$，在采样间隔趋向于0时，$x[n]$就能很好地还原$f(t)$的特征。下面说明，对$x[n]$的离散傅里叶变换$X[k]$在频域上也能很好地还原$f(t)$的特征。
