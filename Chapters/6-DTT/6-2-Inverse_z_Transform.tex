...
要使用$\mathrm{Res}\left(\dfrac{X(1/z)}{z^{n+1}}\right)$计算是非常繁琐的，我们一般会使用留数定理将问题转化为其他极点的留数计算。注意我们并不打算算出复线积分，留数定理是从一些极点到另一些极点的桥梁.
\begin{example}
    设右边信号$x[n]$的z变换为
    ...
    \begin{align*}
        x[n]=-\mathrm{Res}\left(X(z)z^{n-1},\infty\right)=-\frac{1}{2\pi \mathrm{i}}\oint_{C^-_R}X(z)z^{n-1}\,dz
    \end{align*}
    ...
    \begin{align*}
        ...
        &=\frac{(n+1)n}{2}
    \end{align*}
    因此$$x[n]=-\mathrm{Res}\left(X(z)z^{n-1},1\right)=-\frac{(n+1)n}{2}u[n]$$    
\end{example}
...
\begin{example}
    ...
    \end{align*}
    其实，由于$\mathrm{deg}\ z^{n+1}\leq \mathrm{deg}\ (z-1)^3(n\le 0)$，$X(z)z^{n-1}$在$\infty$处为可去奇点，这种情况下可以直接断定无穷远点的留数为0。
\end{example}
