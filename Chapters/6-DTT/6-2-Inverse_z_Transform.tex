...
\begin{example}
    ...
    \end{align*}
    其实，由于$\mathrm{deg}\ z^{n+1}\leq \mathrm{deg}\ (z-1)^3$，$X(z)z^{n-1}$在$\infty$处为可去奇点，这种情况下可以直接断定无穷远点的留数为0。
\end{example}
...
\begin{example}
    ...
    \begin{align*}
        x_{left}[n]&=\mathrm{Res}\left(\frac{z^{n-1}}{z-2},0\right)\\
        &=\frac{1}{2\pi \mathrm{i}}\oint_{C_r}\frac{z^{n-1}}{z-2}\,dz\\
        &=-\mathrm{Res}\left(\frac{z^{n-1}}{z-2},2\right)\\
        &=-2^{n-1}u[-n]\\
        x_{right}[n]&=-\mathrm{Res}\left(-\frac{z^2-z+1}{(z-1)^3}z^{n-1},\infty\right)\\
