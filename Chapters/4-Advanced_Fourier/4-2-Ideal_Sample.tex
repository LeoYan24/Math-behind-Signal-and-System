...
它与上一节中讨论的拉格朗日插值有一定的相似之处。$\mathrm{sinc}$函数在非零整数点处的值为0，在0处的值为1，于是可以直接利用每个数据点处的值限定所还原出信号在这一点的值，换言之，在以下这种公式所代表的插值方式下，数据点处的值是不会产生偏移的，$\mathrm{Sa}$函数同理。不同的是，拉格朗日插值法在数据点以外的位置往往会有误差，而以上插值方法在没有产生混叠现象时是精确的。
...
本小节旨在建立带限周期信号的有限采样公式。工程上，常假设实际信号是带限的，并且只关注有限的时间范围，借助有限采样公式，只需要有限的采样点数（相比之下，无限多的采样点数是无法实现的）即可还原信号。注意符号$T$在本小节中表示信号的周期，而非采样间隔。
