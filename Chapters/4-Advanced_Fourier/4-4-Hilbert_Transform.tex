...
\begin{definition}[希尔伯特变换]
    ...
    \[\mathcal{H} f(x)=\frac{1}{\pi}\text{p.v.}\int_{-\infty}^{\infty}\frac{f(y)}{x-y}\,dy=\left(f*\pv\frac{1}{\pi x}\right)(x)\]
    称$f$与$\mathcal{H} f$为\textbf{希尔伯特变换对}，记为$f\overset{\mathcal{H} }{\longleftrightarrow} \mathcal{H} f$.
\end{definition}

希尔伯特变换既可以看作积分变换，又可以看作滤波器，因为它是函数与分布$\pv 1/\pi t$的卷积（多数文献中略去了$\pv$的符号，但定义是一致的）。希尔伯特变换在信号处理领域有着重要的应用，例如构造解析信号，将在本小节最后介绍。

\begin{remark}
    一些文献中\cite{brad_osgood_ee261}，希尔伯特变换为$-\pv 1/\pi t$与$f(t)$的卷积，我们很快将看到这就是本书中的希尔伯特逆变换，它们没有本质区别；一些文献中\cite{signal_systems}，直接将希尔伯特变换记为$\hat{f}$，这样做有一定的好处，例如书写简便，但它与傅里叶变换的记号冲突，因此本书中不采用这种记号。
\end{remark}
\begin{remark}
...
$\pv\dfrac{1}{x}$作为缓增的正则分布，我们在\refsub{normal_distribution}中推导过其傅里叶变换：
\begin{lralign}
    \mathcal{F} \left[\pv\frac{1}{\pi x}\right](\xi)= -\mathrm{i}\,\text{sgn}(\xi)
    \switch
    \mathcal{F} \left[\pv\frac{1}{\pi x}\right](\omega)= -\mathrm{i}\,\text{sgn}(\omega)
\end{lralign}

借助这个结论，我们可以通过傅里叶变换来研究希尔伯特变换的性质。由分布与函数的卷积定理（\cref{thm:distribution_func_conv}），可以得到：
...
\begin{definition}[希尔伯特逆变换]
    函数$f:\mathbb{R}\to\mathbb{C}$的\textbf{希尔伯特逆变换}定义为
    \[\mathcal{H} ^{-1} f(x)=-\mathcal{H} f(x)=-\frac{1}{\pi}\text{p.v.}\int_{-\infty}^{\infty}\frac{f(y)}{x-y}\,dy=-\left(f*\pv\frac{1}{\pi x}\right)(x)\]
\end{definition}
...
\begin{example}[三角函数的希尔伯特变换]
    ...
    \end{lralign}
    因此余弦函数的希尔伯特变换的傅里叶变换为（假设$2\pi f_0=\omega_0>0$）：
    \begin{lralign}
...
\begin{example}[三角函数的希尔伯特变换，直接计算]
    利用狄利克雷积分（\cref{thm:dirichlet_int}）：
    \[\int_{-\infty}^{\infty}\frac{\sin ax}{x}\,dx=\pi,\quad a>0\]
    也可以直接计算三角函数的希尔伯特变换，这里以正弦函数为例：
    \begin{align*}
        ...
        &\qquad \left.+\int_{-\infty}^{-\epsilon}\frac{\sin(\omega_0 t)\cos(\omega_0 u)-\cos(\omega_0 t)\sin(\omega_0 u)}{u}\,du\right)\\
        ...
        &\qquad \left.-\cos(\omega_0 t)\int_{\epsilon}^{\infty}\frac{\sin(\omega_0 u)}{u}\,du-\cos(\omega_0 t)\int_{-\infty}^{-\epsilon}\frac{\sin(\omega_0 u)}{u}\,du\right)\\
...
简洁起见，本小节主要使用频率形式的傅里叶变换。如果$f\in L^2(\mathbb{R})$，即$f(t)$是一个能量有限信号，则$\mathcal{H} f\in L^2(\mathbb{R})$，并且$\|\mathcal{H} f\|_2=\|f\|_2$，进一步，$L^2(\mathbb{R})$上的\textbf{希尔伯特变换是保内积的}：
\begin{theorem}
...
\begin{proof}
    ...
    \begin{align*}
        ...
        &=0
    \end{align*}
    最后一个等号是因为实函数的傅里叶变换具有共轭对称性，$|\mathcal{F} f(\xi)|$是偶函数，因此$\text{sgn}(\xi)|\mathcal{F} f(\xi)|^2$是奇函数。
\end{proof}

此外，希尔伯特变换具有奇偶对称性：
\begin{theorem}
    $f$与$\mathcal{H}f$的奇偶性相反，即：
    \begin{itemize}
...
\begin{proof}
    ...
    类似地，如果$f$是奇函数，则$\mathcal{F} f$也是奇函数，$\mathcal{F} [\mathcal{H} f](\xi)=-\mathrm{i}\,\text{sgn}(\xi)\mathcal{F} f(\xi)$是偶函数，因此$\mathcal{H} f$是偶函数。直接使用希尔伯特变换的定义也可验证这一性质。
\end{proof}
...
设$h(t)$是因果线性时不变系统的单位冲激响应，频率响应为$H(\omega)$.由$h(t)=u(t)h(t)$，可得：
\begin{align*}
    H(\omega)&=\mathcal{F} [h(t)u(t)](\omega)\\
    &=\frac{1}{2\pi}H(\omega)*\left(\pi\delta(\omega)+\pv\frac{1}{\mathrm{i}\omega}\right)\\
    &=\frac{1}{2}H(\omega)+\pv\frac{1}{2\pi \mathrm{i}\omega}*H(\omega)
\end{align*}

考察上式的实部和虚部，记$H(\omega)=R(\omega)+\im I(\omega)$，则有：
\begin{align*}
    R(\omega)&=\frac{1}{2}R(\omega)-\pv\frac{1}{2\pi \omega}*I(\omega)\\
    I(\omega)&=\frac{1}{2}I(\omega)+\pv\frac{1}{2\pi \omega}*R(\omega)
\end{align*}
整理以上两式即得：
\begin{theorem}[希尔伯特变换关系]
    设$h(t)$为因果线性时不变系统的单位冲激响应，$H(\omega)=R(\omega)+\im I(\omega)$为系统的频率响应，则$R(\omega)$与$I(\omega)$构成希尔伯特变换对，即：
    \begin{align*}
    &R(\omega)=-\pv\frac{1}{\pi \omega}*I(\omega)=\mathcal{H} ^{-1}I(\omega)\\
    &I(\omega)=\pv\frac{1}{\pi \omega}*R(\omega)=\mathcal{H} R(\omega)
\end{align*}
