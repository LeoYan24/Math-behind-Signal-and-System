...
在\refsub{poisson_summation}中，我们指出，周期函数可以视作施瓦兹函数的周期延拓，并且周期函数的傅里叶级数的系数正比于紧支函数的傅里叶变换在离散点处的取值。因此，我们希望周期函数的卷积的傅里叶系数也具有类似卷积定理的性质。事实也确实如此：
...
\begin{proof}
    ...
    \end{align*}
    因此$f*g$是有界的。其次，由于$L^2$空间中的含参变量积分具有一致连续性（目前读者掌握的分析工具尚不足以证明此命题），卷积$f*g$也是一致连续的。连续函数当然是可积的，因此
    $$f*g\in L^2([0,T])$$
