...
不考虑收敛性的问题，当函数$f$有周期$T$时，$f$与另一个函数$g$的卷积$f*g$也具有周期$T$。这是因为，根据我们在\refsub{intuition_of_convolution}中给出的卷积的性质，
...
\begin{definition}[周期函数的卷积]\label{def:period_conv}
...
\begin{proposition}[卷积的线性性、结合律、交换律]
    ...
    \end{align*}    
\end{proposition}
它的证明与\cref{prop:conv_prop}类似，不再赘述。
...
不过，周期函数的卷积也与经典的卷积有一些差别。例如，在函数的平移和伸缩变换下，卷积不再具有类似\cref{prop:conv_signal_shift}中的性质，因为这两个性质的证明都依赖于无穷的积分区间。

在\refsub{poisson_summation}中，我们指出，周期函数可以视作施瓦兹函数的周期延拓，并且周期函数的傅里叶级数的系数正比于紧支函数的傅里叶变换在离散点处的取值。因此，我们希望周期函数的卷积的傅里叶系数也具有类似卷积定理的性质。事实也确实如此。我们先引入以下结论：
\begin{claim}
    设$1\le p<\infty$，$1/p+1/q=1$，$f\in L^p(\mathbb{R})$，$g\in L^q(\mathbb{R})$，则$f*g$在$\mathbb{R}$上有限且一致连续。
\end{claim}
...
\end{theorem}
\begin{proof}
    为了验证$f*g\in L^2([0,T])$，我们说明这个函数是一致连续且有界的。首先，
    \begin{align*}
    ...
    \end{align*}
    因此$f*g$是有界的。其次，$L^2$空间中，卷积$f*g$是一致连续的，因此是可积的，即
    $$f*g\in L^2([0,T])$$

    记$f,g$的基波角频率为$\omega=\dfrac{2\pi}{T}$，则
    \begin{align*}
        c_n&=\frac{1}{T}\int_0^{T}(f*g)(x)\mathrm{e}^{-\mathrm{i}n\omega x}\,dx\\
        &=\frac{1}{T^2}\int_0^{T}\mathrm{e}^{-\mathrm{i}n\omega x}\,dx\int_0^{T}f(y)g(x-y)\,dy\\
        &=\frac{1}{T^2}\int_0^{T}f(y)\,dy\int_0^{T}g(x-y)\mathrm{e}^{-\mathrm{i}n\omega x}\,dx\\
        &=\frac{1}{T^2}\int_0^{T}f(y)\,dy\int_{-y}^{T-y}g(z)\mathrm{e}^{-\mathrm{i}n\omega (z+y)}\,dz&(z=x-y)\\
        &=\frac{1}{T^2}\int_0^{T}f(y)\mathrm{e}^{-\mathrm{i}n\omega y}\,dy\int_0^{T}g(z)\mathrm{e}^{-\mathrm{i}n\omega z}\,dz\\
        &=\left(\frac{1}{T}\int_0^{T}f(y)\mathrm{e}^{-\mathrm{i}n\omega y}\,dy\right)\left(\frac{1}{T}\int_0^{T}g(z)\mathrm{e}^{-\mathrm{i}n\omega z}\,dz\right)\\
        &=a_n b_n
    \end{align*}
\end{proof}
...
\begin{align*}
    \|f(\cdot) w(\cdot-\tau)\|_1&=\int_{-\infty}^{\infty}|f(t)w(t-\tau)|\,dt\\
...
\begin{itemize}
    ...
    \item 时域截取特性：窗函数$w$紧支或快速衰减
\end{itemize}

归一化条件是必要的，否则窗函数$w$的若干倍仍为窗函数，但$\alpha w(\alpha\in\mathbb{R})$不具有单独的研究价值。我们很快会看到选取$L^2$范数作为归一化条件带来的方便。
...
以上过程对$f(t)w(t-\tau)$使用了傅里叶反演公式并忽略了在不连续点的取值问题。前面已经验证$f(t)w(t-\tau)\in L^1(\mathbb{R})$，但是\refsub{fourier_inversion}中傅里叶反演公式所需的其他条件需要对$f,w$做出更严格的假设，这里不再赘述。
\end{proof}

即使不要求窗函数归一化，我们也能得到类似的公式，但会引入额外的系数，这解释了为什么我们以$L^2$范数归一化窗函数。

在适当的伸缩变换下，可以保证窗函数满足能量归一化的条件，而不改变时域截取的特性。那么，应该如何选取窗函数的宽度呢？为了回答这个问题，我们首先需要研究窗的在宽度变化时，短时傅里叶变换的行为特性。
...
频率分辨率是如何降低的呢？根据卷积定理，有：\begin{lralign}
    \mathrm{STFT} _{f,w}(\tau,\xi)&=\mathcal{F} [f(t)w(t-\tau)](\xi)\\
...
我们知道，卷积是一种起“平均化”作用的运算，与$\mathcal{F} f$卷积的函数越集中（类似$\delta$分布），平均化作用越弱，卷积所得函数越接近$\mathcal{F} f$本身；反之，与$\mathcal{F} f$卷积的函数越分散（类似$\mathds{1}$），平均化作用越强，卷积所得函数越平滑。因此，窗函数变窄时，尽管很容易指出频率特性所反映的时域信号位置，但$\mathcal{F} w_a$变宽，\textbf{频谱受平均化作用，频率分辨率降低}。
...
为了兼顾时间分辨率和频率分辨率，一方面，我们可以探究对于特定的信号处理场景，通常应该选取怎样的窗函数；另一方面，我们可以对窗函数做多尺度分析，即对不同的频率成分使用不同宽度的窗函数。例如，对高频成分使用窄窗以提高时间分辨率，对低频成分使用宽窗以提高频率分辨率。这种方法就是\textbf{小波变换}(wavelet transform,WT)的思想。
...
\begin{definition}[小波变换]
    ...
    \begin{align*}
        \mathrm{WT} _{f,\psi}(a,b)=\int_{-\infty}^{\infty}f(t)\overline{\psi_{a,b}(t)}\,dt=(f,\psi_{a,b})
    \end{align*}
    其中$(\cdot,\cdot)$为$L^2(\mathbb{R})$空间中的内积。通过变量代换$u=\dfrac{t-b}{a}$，小波变换也可写为：
    \begin{align*}
...
小波母函数需满足\textbf{容许性条件}(admissibility condition)：
...
我们很快会看到，这一条件保证了小波变换可逆，并使得小波变换的普朗歇尔公式成立。

容许性条件蕴含了小波母函数的直流分量为0：
...
\begin{proof}
    ...
    \end{align*}
    则由于$\hat{\psi}$连续（见\refsub{introduction_of_fourier}），在$\xi=0$的小邻域$(-\delta,\delta)$内，$|\hat{\psi}(\xi)|\geq \dfrac{|\hat{\psi}(0)|}{2}>0$，从而
    \begin{align*}
        C_{\psi}&=\int_{-\infty}^{\infty}\frac{|\hat{\psi}(\xi)|^2}{|\xi|}\,d\xi\\
        &\geq \int_{-\delta}^{\delta}\frac{|\hat{\psi}(\xi)|^2}{|\xi|}\,d\xi&(\delta>0)\\
        &\geq \int_{-\delta}^{\delta}\frac{|\hat{\psi}(0)|^2}{4|\xi|}\,d\xi=\infty
    \end{align*}
    矛盾。
\end{proof}

\begin{theorem}[小波变换的普朗歇尔公式]
    设$f,g\in L^2(\mathbb{R})$，$\psi$为满足容许性条件的小波母函数，则
    \[(f,g)=\frac{1}{C_{\psi}}\int_{-\infty}^{\infty}a^{-2}\,da\int_{-\infty}^{\infty}\mathrm{WT} _{f,\psi}(a,b)\overline{\mathrm{WT} _{g,\psi}(a,b)}\,db\]
\end{theorem}
\begin{proof}
    \refsub{fourier_operation_properties}中的普朗歇尔定理表明：
    \begin{align*}
        (f,g)=(\hat{f},\hat{g})=\int_{-\infty}^{\infty}\hat{f}(\xi)\overline{\hat{g}(\xi)}\,d\xi
    \end{align*}
    其中$\hat{f},\hat{g}$分别为$f,g$的傅里叶变换。另一方面，考察右式内层的积分：
    \begin{align*}
        \int_{-\infty}^{\infty}\mathrm{WT} _{f,\psi}(a,b)\overline{\mathrm{WT} _{g,\psi}(a,b)}\,db=\left( \mathrm{WT} _{f,\psi}(a,\cdot),\mathrm{WT} _{g,\psi}(a,\cdot)\right)
    \end{align*}
    ...
    \begin{align*}
        ...
        =&\int_{-\infty}^{\infty}\hat{f}(\xi)\overline{\hat{g}(\xi)}\,d\xi\\
        =&(f,g)
    \end{align*}
\end{proof}
...
\begin{proof}
    ...
    \begin{align*}
        ...
        =&\left( \mathrm{WT} _{f,\psi}(a,\cdot),\overline{\psi_{a,\cdot}(t)}\right)\\
        =&\left( \mathcal{F} _b[\mathrm{WT} _{f,\psi}(a,b)](\xi),\mathcal{F} _b[\overline{\psi_{a,b}(t)}](\xi)\right)\\
    ...
    \begin{align*}
        ...
        =&\int_{-\infty}^{\infty}\hat{f}(\xi)\mathrm{e}^{2\pi \mathrm{i}\xi t}\,d\xi\\
        =&f(t)
    \end{align*}
\end{proof}
...
\begin{example}[Haar小波]
    ...
    \begin{align*}
        ...
        &=\frac{1-\mathrm{e}^{-\pi \mathrm{i} \xi}}{2\pi \mathrm{i} \xi}-\frac{\mathrm{e}^{-\pi \mathrm{i} \xi}-\mathrm{e}^{-2\pi \mathrm{i} \xi}}{2\pi \mathrm{i} \xi}\\
        &=\frac{1-2\mathrm{e}^{-\pi \mathrm{i} \xi}+\mathrm{e}^{-2\pi \mathrm{i} \xi}}{2\pi \mathrm{i} \xi}\\
        &=\frac{(1-\mathrm{e}^{-\pi \mathrm{i} \xi})^2}{2\pi \mathrm{i} \xi}\\
    ...
    \end{align*}
    在最后一个等号处，我们利用了
    $$\int_{0}^{\infty}\dfrac{\sin^4(x)}{x^3}\,dx=\dfrac{\ln 2}{2}$$
    因此，Haar小波满足容许性条件。
\end{example}
