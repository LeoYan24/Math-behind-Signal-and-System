...
\begin{example}[矩形函数]
    ...
    \begin{lralign}
    \mathcal{F} \left(\chi_{[-T/2,T/2]}\right)(\xi)&=T\mathrm{sinc}(T\xi)
\switch
    \mathcal{F} \left(\chi_{[-T/2,T/2]}\right)(\omega)&=T\mathrm{Sa}\left(\frac{T\omega}{2}\right)
    \end{lralign}
    因此，n维矩形函数的傅里叶变换为：
    \begin{lralign}
    \mathcal{F} \chi_I(\boldsymbol{\xi})&=T^n \prod_{j=1}^n \mathrm{sinc}(T\xi_j)
\switch
    \mathcal{F} \chi_I(\boldsymbol{\omega})&=T^n \prod_{j=1}^n \mathrm{Sa}\left(\frac{T\omega_j}{2}\right)
    \end{lralign}
\end{example}
...
多维傅里叶变换的运算性质相较一维比较丰富，本小节中将分条列举这些性质。
...
\end{proposition}
\begin{proof}
    类似一维的情况（\cref{thm:FT_dual}），带入定义可立即得到：
    \begin{lralign}
        \mathcal{F} f^-=\mathcal{F} (f^-)=\mathcal{F} ^{-1} f
    \switch
        \mathcal{F} f^-=(2\pi)^n \mathcal{F} ^{-1} f
...
\switch
    \mathcal{F} f^-(\boldsymbol{\omega})&=\int_{\mathbb{R}^n}f(-\mathbf{x})\mathrm{e}^{-\mathrm{i} \mathbf{x}\cdot\boldsymbol{\omega}}\,d\mathbf{x}\\
    &=\overline{\int_{\mathbb{R}^n}f(\mathbf{x})\mathrm{e}^{\mathrm{i} \mathbf{x}\cdot\boldsymbol{\omega}}\,d\mathbf{x}}\\
    &=(2\pi)^n \overline{\mathcal{F} f(\boldsymbol{\omega})}
    \end{lralign}
\end{proof}
...
\begin{proposition}[伸缩定理]
    ...
    \end{lralign}
    特别地，当$A$是对角阵时，设$A=\mathrm{diag}(a_1,a_2,\cdots,a_n)$，则有：
    \begin{lralign}
...
\begin{definition}[场论中的算子]
    ...
    \begin{itemize}
        ...
        \item 散度：$\nabla\cdot \mathbf{F}=\mathrm{div}\mathbf{F}$，其中$\mathbf{F}=(F_1,F_2,\cdots,F_n)$
        \item 旋度（仅在$\mathbb{R}^3$中定义）：$\nabla\times \mathbf{F}=\mathrm{curl}\mathbf{F}$（一些文献中也将旋度算子记为$\mathrm{rot}$）
    \end{itemize}
\end{definition}

\begin{corollary}[场论中的微分性质]
    由微分性质，我们依次对比向量值函数的各个分量。对于梯度$\nabla f=\mathrm{grad}f$，有：
    \begin{lralign}
...
\switch
    ...
    \end{lralign}
    对于散度$\nabla\cdot \mathbf{F}=\mathrm{div}\mathbf{F}$，其中$\mathbf{F}=(F_1,F_2,\cdots,F_n)$，有：
    \begin{lralign}
...
\switch
    ...
    \end{lralign}
    在$\mathbb{R}^3$中，对于旋度$\nabla\times \mathbf{F}=\mathrm{curl}\mathbf{F}$，有：
    \begin{lralign}
...
换言之，频域上，$\nabla$算子相当于$2\pi\mathrm{i}\boldsymbol{\xi}=\mathrm{i}\boldsymbol{\omega}$。
...
\begin{lralign}
    ...
    \end{lralign}
\ding{173}频域卷积：\\
仿照\cref{thm:conv}的证明。根据时域卷积定理$\mathcal{F} (f*g)=\mathcal{F} f\mathcal{F} g$，
...
与一维的情况下的\cref{thm:plancherel}类似，我们给出更一般的普朗歇尔定理，它实际上指出空间域与变换域上的内积的数量关系。
...
\end{theorem}
\begin{proof}
    设$f,g\in L^1(\mathbb{R}^n)\cap L^2(\mathbb{R}^n)$，则有：
    \begin{lralign}
...
容易验证$\mathcal{S} $上的卷积总是有定义的；用类似于\cref{thm:schwartz_close}的证明方法，可以证明$\mathcal{S} $在傅里叶变换下是封闭的，我们仍然定义$\langle \mathcal{F} T,\varphi\rangle=\langle T,\mathcal{F} \varphi\rangle$，从而$\mathcal{T} $在傅里叶变换下也是封闭的。典型的施瓦兹函数例如高斯函数$f(\mathbf{x})=\mathrm{e}^{-\pi |\mathbf{x}|^2}$。
...
\begin{example}[平移算子]
    ...
    \end{align*}
    因此我们定义：
    \[\langle \tau_{\mathbf{b}} T,\quad \varphi\rangle=\langle T,\tau_{-\mathbf{b}}\varphi\rangle,T\in\mathcal{T} ,\varphi\in\mathcal{S} \]
\end{example}
...
\begin{example}[伸缩算子]
    ...
    \end{align*}
    因此我们定义：
    \[\langle \sigma_A T,\varphi\rangle=\frac{1}{|\det A|}\langle T,\sigma_{A^{-1}}\varphi\rangle,\quad T\in\mathcal{T} ,\varphi\in\mathcal{S} \]
\end{example}
在一维情况下，我们有：

\[\langle \tau_a T,\varphi\rangle=\langle T,\tau_{-a}\varphi\rangle,\langle\sigma_{a} T,\varphi\rangle=\frac{1}{|a|}\langle T,\sigma_{1/a}\varphi\rangle,\quad T\in\mathcal{T} ,\varphi\in\mathcal{S} \]
...
\begin{definition}[多维狄拉克分布]
    ...
    0,&\mathbf{x}\neq \mathbf{x}_0
    \end{cases},\quad \int_{\mathbb{R}^n}\delta(\mathbf{x})\,d\mathbf{x}=1\]
...
\begin{proof}
    分布与函数的卷积定义为：
    \[\langle f*T,\varphi\rangle=\langle T,(f^-)*\varphi\rangle,\quad f,\varphi\in\mathcal{S} ,T\in\mathcal{T} \]
...
\switch
    ...
    再利用傅里叶变换的频移性质，得到：
    \begin{lralign}
        \mathcal{F} [\mathrm{e}^{2\pi \mathrm{i} \boldsymbol{\xi}_0\cdot\mathbf{x}}](\boldsymbol{\xi})&=\delta_{\boldsymbol{\xi}_0}
\switch
        \mathcal{F} [\mathrm{e}^{\mathrm{i} \boldsymbol{\omega}_0\cdot\mathbf{x}}](\boldsymbol{\omega})&=(2\pi)^n \delta_{\boldsymbol{\omega}_0}
    \end{lralign}
    利用欧拉公式求得三角函数的傅里叶变换：
    \begin{lralign}
        \mathcal{F} [\cos(2\pi \boldsymbol{\xi}_0\cdot\mathbf{x})](\boldsymbol{\xi})&=\frac{1}{2}[\delta_{\boldsymbol{\xi}_0}+\delta_{-\boldsymbol{\xi}_0}]
\switch
        \mathcal{F} [\cos(\boldsymbol{\omega}_0\cdot\mathbf{x})](\boldsymbol{\omega})&=\frac{(2\pi)^n}{2}[\delta_{\boldsymbol{\omega}_0}+\delta_{-\boldsymbol{\omega}_0}]
    \end{lralign}
    以及
    \begin{lralign}
        \mathcal{F} [\sin(2\pi \boldsymbol{\xi}_0\cdot\mathbf{x})](\boldsymbol{\xi})&=\frac{1}{2\mathrm{i}}[\delta_{\boldsymbol{\xi}_0}-\delta_{-\boldsymbol{\xi}_0}]
\switch
        \mathcal{F} [\sin(\boldsymbol{\omega}_0\cdot\mathbf{x})](\boldsymbol{\omega})&=\frac{(2\pi)^n}{2\mathrm{i}}[\delta_{\boldsymbol{\omega}_0}-\delta_{-\boldsymbol{\omega}_0}]
    \end{lralign}
\end{corollary}
...
\begin{definition}[格]
    ...
    \[\mathbf{p}=p_1\mathbf{u}_1+p_2\mathbf{u}_2,\quad p_1,p_2\in\mathbb{Z}\]
    ...
    \[\mathbf{u}_1=A \mathbf{e}_1,\quad \mathbf{u}_2=A \mathbf{e}_2\]
    其中$\mathbf{e}_1,\mathbf{e}_2$为标准正交基。作为列向量，格点可以表示为
    \[\mathbf{p}=A\mathbf{n},\quad \mathbf{n}\in\mathbb{Z}^2\]
...
\begin{definition}[二维傅里叶级数]
    ...
    \[f(x_1,x_2)=f(x_1+T_1,x_2)=f(x_1,x_2+T_2),\quad \forall (x_1,x_2)\in\mathbb{R}^2\]
...
\end{theorem}
\begin{proof}
    仿照一维情况下的证明（见\refsub{poisson_summation}），令$\Phi(\mathbf{x})=(\shah *\varphi)(\mathbf{x})=\sum_{\mathbf{k}\in\mathbb{Z}^2}\varphi(\mathbf{x}-\mathbf{k})$，则$\Phi$为周期为1的二维函数，可以展开为傅里叶级数：
...
\begin{definition}[对偶基与对偶线性泛函]
    ...
    $\delta^i_{j}$为克罗内克$\delta$符号。我们将$\mathbf{u}^*_1,\mathbf{u}^*_2$称为$\mathbf{u}_1,\mathbf{u}_2$的\textbf{对偶基}。\textbf{对偶基生成了对偶格}。此外，与对偶基做内积，定义了内积空间上唯一一组满足正交归一化条件，即$f_i(\mathbf{u}_j)=\delta^i_{j}$的线性泛函，这组线性泛函称为\textbf{对偶线性泛函}。
\end{definition}

$\shah_{\mathcal{L} }$的$\delta$分布位于格$\mathcal{L} $的格点上，而其傅里叶变换$\mathcal{F} \shah_{\mathcal{L} }$的$\delta$分布位于对偶格$\mathcal{L} ^*$的格点上。这为我们展示了一种理解空间域与频域关系的方式，并从更高的视角解释了我们在\refsub{fourier_of_dirac_comb}最后所发现的现象，即一维狄拉克梳状分布经过傅里叶变换，$\delta$的位置具有“倒数”关系。
...
\begin{definition}[二维带限函数]
    ...
    使得$\mathrm{supp}(\mathcal{F}f)\subset B$，就说$f$为二维\textbf{带限函数}(bandlimited function)，带宽为$(\xi_{m1},\xi_{m2})$。
\end{definition}
特别地，我们考虑
$$\mathrm{supp}\mathcal{F} f\subset\left\{( \xi_1,\xi_2)\left| |\xi_1|\leq \dfrac{1}{2},|\xi_2|\leq \dfrac{1}{2}\right.\right\}$$
的情况。由恒等式
\[\mathcal{F} f(\boldsymbol{\xi})=\chi_{[-1/2,1/2]^2}(\boldsymbol{\xi})(\mathcal{F} f(\boldsymbol{\xi})*\shah)\]
取傅里叶逆变换，由\refsub{multi_fourier}中的结果可知$\mathcal{F} ^{-1}\chi_{[-1/2,1/2]}(\mathbf{x})=\mathrm{sinc}(\mathbf{x})$，我们得到：\begin{align*}
    ...
    &=\mathrm{sinc}(\mathbf{x})*(f(\mathbf{x})*\shah(\mathbf{x}))\\
    &=\sum_{\mathbf{k}\in\mathbb{Z}^2}f(\mathbf{k})\mathrm{sinc}(x_1-k_1)\mathrm{sinc}(x_2-k_2)
\end{align*}
最后一行用到了卷积的结合律，见\refsub{properties_of_convolution}。综上得到以下引理：
\begin{lemma}[方形采样公式]
    设$$f\in L^1(\mathbb{R}^2),\mathrm{supp}(\mathcal{F} f)\subset\left\{( \xi_1,\xi_2)\left| |\xi_1|\leq \dfrac{1}{2},|\xi_2|\leq \dfrac{1}{2}\right.\right\}$$
    则有
    \[f(\mathbf{x})=\sum_{\mathbf{k}\in\mathbb{Z}^2}f(\mathbf{k})\mathrm{sinc}(x_1-k_1)\mathrm{sinc}(x_2-k_2)\]
\end{lemma}

更一般地，我们考虑$\mathrm{supp}(\mathcal{F} f)$属于格$\mathcal{L} =A(\mathbb{Z}^2),A=[\mathbf{u}_1,\mathbf{u}_2]$的某个基本平行四边形内。显而易见，带限等价于存在这样一个平行四边形，使得频域信号$\mathcal{F} f$支于它。由于我们通常考虑实信号，不妨设这个基本平行四边形是关于原点对称的。
...
令$g(\mathbf{x})=f(A^{-T}\mathbf{x})$，则$\mathrm{supp}(\mathcal{F} g)\in[-1/2,1/2]^2$，因此可以应用方形采样公式：
\[g(\mathbf{x})=\sum_{\mathbf{k}\in\mathbb{Z}^2}g(\mathbf{k})\mathrm{sinc}(x_1-k_1)\mathrm{sinc}(x_2-k_2)\]
将$g$的定义代入上式，得到：
\[f(A^{-T}\mathbf{x})=\sum_{\mathbf{k}\in\mathbb{Z}^2}f(A^{-T}\mathbf{k})\mathrm{sinc}(x_1-k_1)\mathrm{sinc}(x_2-k_2)\]

以上结果可以改写为\textbf{格采样公式}：
\begin{theorem}[格采样公式]
    设$$f\in L^1(\mathbb{R}^2),\mathrm{supp}(\mathcal{F} f)\subset\left\{( \xi_1,\xi_2)\left| \mathbf{\xi}=A\boldsymbol{\eta},\boldsymbol{\eta}\in[-1/2,1/2]^2\right.\right\}$$
    则有
    \[f(\mathbf{y})=\sum_{k_1,k_2\in\mathbb{Z}}f(k_1\mathbf{u}^*_1+k_2\mathbf{u}^*_2)\mathrm{sinc}(\mathbf{u}_1\cdot \mathbf{y}-k_1)\mathrm{sinc}(\mathbf{u}_2\cdot \mathbf{y}-k_2)\]
\end{theorem}
\begin{proof}
    令$\mathbf{y}=A^{-T}\mathbf{x}$，则$\mathbf{x}=A^T\mathbf{y}$，于是
    \[x_1=\mathbf{u}_1\cdot \mathbf{y},\quad x_2=\mathbf{u}_2\cdot \mathbf{y}\]
    对$A^{-T}\mathbf{k}$，我们知道$A^{-T}$的行向量分别为$\mathbf{u}^*_1,\mathbf{u}^*_2$，因此$A^{-T}\mathbf{k}=k_1\mathbf{u}^*_1+k_2\mathbf{u}^*_2$。综上得到：
    \begin{align*}
        f(\mathbf{y})&=\sum_{\mathbf{k}\in\mathbb{Z}^2}f(A^{-T}\mathbf{k})\mathrm{sinc}(\mathbf{u}_1\cdot \mathbf{y}-k_1)\mathrm{sinc}(\mathbf{u}_2\cdot \mathbf{y}-k_2)\\
        &=\sum_{k_1,k_2\in\mathbb{Z}}f(k_1\mathbf{u}^*_1+k_2\mathbf{u}^*_2)\mathrm{sinc}(\mathbf{u}_1\cdot \mathbf{y}-k_1)\mathrm{sinc}(\mathbf{u}_2\cdot \mathbf{y}-k_2)
    \end{align*}
\end{proof}
