...
\end{proposition}
\begin{proof}
    类似一维的情况（见\refsub{fourier_operation_properties}），带入定义可立即得到：
    \begin{lralign}
        \mathcal{F} f^-=\mathcal{F} (f^-)=\mathcal{F} ^{-1} f
    \switch
        \mathcal{F} f^-=(2\pi)^n \mathcal{F} ^{-1} f
...
\begin{lralign}
    ...
    \end{lralign}
\ding{173}频域卷积：\\
仿照\refsub{properties_of_convolution}中的证明。根据时域卷积定理$\mathcal{F} (f*g)=\mathcal{F} f\mathcal{F} g$，
...
容易验证$\mathcal{S} $上的卷积总是有定义的；用类似于\refsub{fourier_close}中的方法，可以证明$\mathcal{S} $在傅里叶变换下是封闭的，我们仍然定义$\langle \mathcal{F} T,\varphi\rangle=\langle T,\mathcal{F} \varphi\rangle$从而$\mathcal{T} $在傅里叶变换下也是封闭的。典型的施瓦兹函数例如高斯函数$f(\mathbf{x})=\mathrm{e}^{-\pi |\mathbf{x}|^2}$。
