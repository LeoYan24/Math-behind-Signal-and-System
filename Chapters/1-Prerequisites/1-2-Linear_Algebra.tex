...
显然，子空间的维数低于母空间的维数。我们将在\refsub{infinite_linear}中讨论无限维线性空间的内容。
...
\begin{definition}[线性映射]
    ...
    \[T(\alpha \mathbf{u}+\beta \mathbf{v})=\alpha T(\mathbf{u})+\beta T(\mathbf{v}),\quad\forall \mathbf{u},\mathbf{v}\in V,\forall \alpha,\beta\in F\]
...
\end{definition}
在数学分析中，一个实数列的收敛性等价于该序列是柯西列，这实际上是依赖于实数的完备性的。柯西列描述了一个“应当收敛”的序列所具有的性质，而不依赖于最终的极限是否在我们所讨论的空间中。更一般地，我们提出度量空间中完备性的定义：

\begin{definition}[完备度量空间]
    如果度量空间中所有柯西列都有极限，那么这个空间称为\textbf{完备度量空间}。
\end{definition}

稍后我们将给出一个不完备的线性赋范空间的例子，它同时也是不完备的度量空间的例子。
...
\begin{example}
    $\mathbb{R}^n$中向量的模长$|\mathbf{v}|=\sqrt{\sum_{k=1}^n v_k^2}$是线性空间$\mathbb{R}^n$上的范数。
\end{example}

在线性空间$V$上定义范数后，可以自然地诱导出一种度量：
...
\begin{definition}[Lipschitz连续]
    ...
    $$\exists L>0,\forall x,y\in X,\quad |f(x)-f(y)|\leq L\|x-y\|$$
\end{definition}
...
\begin{proof}
    ...
    $$\|u_n\|=1,\quad\lim_{n\to \infty}\Phi(u_n)=\infty$$
...
\begin{definition}[希尔伯特空间]\label{def:hilbert_space}
...
\begin{proposition}[三角不等式]
    \[\forall \mathbf{a,b}\in V,\quad\|\mathbf{a+b}\|\leq\|\mathbf{a}\|+\|\mathbf{b}\|\]
\end{proposition}
...
对于V，我们称向量列
$\lbrace\mathbf{v}_i\rbrace_{i=1}^{\infty}$是V的一组基，如果
\begin{itemize}
    \item 线性无关性：任取基中的有限个向量，它们是线性无关的
    \item 有限生成性：任取向量$\mathbf{w} \in V$，存在有限个向量
          $V'=\lbrace\mathbf{v}_1,\mathbf{v}_2,\dots,\mathbf{v}_r\rbrace\subset V $，$\mathbf{w}$可以用$V'$线性表出
\end{itemize}
\end{definition}
...
\begin{example}
    ...
    \[l^2=\left\{\mathbf{x}=(x_1,x_2,\ldots)\left|\sum_{i=1}^{\infty}|x_i|^2<\infty\right.\right\}\]
    $l^2$空间是行向量（或列向量）的一种推广，其中标准正交基推广为只有一个元素为$1$的数列之集合，但它不构成$l^2$空间的基，因为它不满足有限生成性。
\end{example}
\begin{example}
...
\refsec{Real_Analysis}将给出更多无穷维线性空间的例子，它们当中很多都是完备的线性赋范空间。我们并不总是能够找到函数空间的基，但使用正交函数系作为基函数系是简单且，从而将函数展开为这些基函数的线性组合，这就是傅里叶级数展开的基本思想，见\refsub{orthognal}。