...
\end{definition}
注意到$\mathrm{Re}(z)>0$时，$|x^{z-1}\mathrm{e}^{-x}|<t^{\mathrm{Re}(z)-1}$，而$t^{\mathrm{Re}(z)-1}$在$[0,\infty)$上可积，因此伽马函数的积分定义在右半平面$\mathrm{Re}(z)>0$上收敛。事实上它是$\mathrm{Re}(z)>0$上的全纯函数，全纯的概念，见\ref{sec:Complex}。
